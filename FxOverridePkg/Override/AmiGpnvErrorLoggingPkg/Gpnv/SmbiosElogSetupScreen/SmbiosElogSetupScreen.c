...
EFI_GUID gEfiSmbiosElogSupportGuid;
EFI_GUID gEventLogsDynamicGuid;
...
// fill event log info
    ...
    Status = gBS->InstallProtocolInterface(&TempHandle,
        &gEventLogsDynamicGuid, EFI_NATIVE_INTERFACE, &DynamicCallbackInfo );
...
// Make sure HII Handle for Strings in Event Log Page is present
  ...
  if (mPrivateData.HiiHandle == NULL) {
      ...
      Status = gBS->LocateProtocol( &gEventLogsHiiHandleGuidx,
                                    NULL,
...
// Allocate Buffer
  ...
  #if SMBIOSELOG_GET_SMBIOS_DEVICE_STRING
  SmbiosString = (CHAR16 *)AllocateZeroPool(STRING_BUFFER_LENGTH);
  #endif
...
// If not an EFI error, do work for legacy (0x80 and above is OEM)
  ...
  //TH0053 if (EventPtr->EventType < 0x80) {
  //TH0095 if (EventPtr->EventType < 0x85) {  //TH0053
  if (EventPtr->EventType < 0x86) { //TH0095
    switch (EventPtr->EventType) {
    	//TH0095 >>>>
        case EFI_EVENT_LOG_TYPE_CLEAR_CMOS:
            ClassSubClassToken = STRING_TOKEN (STR_EVENT_LOG_TYPE_CLEAR_CMOS);
        break;
      //TH0095 <<<<   
        ...
        case EFI_EVENT_LOG_TYPE_END_OF_LOG:
            ...
            break;
        //TH0053>>>
        //0x80 and above is OEM defined log;  
        case EFI_EVENT_LOG_TYPE_OEM_IDE_DEVICE_FAIL:
            ClassSubClassToken = STRING_TOKEN (STR_EVENT_LOG_TYPE_IDE_DEVICE_FAILURE);
            break;
        case EFI_EVENT_LOG_TYPE_OEM_FLASH_UPDATE_FAILED:
            ClassSubClassToken = STRING_TOKEN (STR_EVENT_LOG_TYPE_FLASH_UPDATE_FAILED);
            break;
        case EFI_EVENT_LOG_TYPE_PCI_BUS_OUT_OF_RESOURCES:
            ClassSubClassToken = STRING_TOKEN (STR_EVENT_LOG_TYPE_PCI_BUS_OUT_OF_RESOURCES);
            break;
        case EFI_EVENT_LOG_TYPE_LEGACY_OPROM_NO_SPACE:
            ClassSubClassToken = STRING_TOKEN (STR_EVENT_LOG_TYPE_LEGACY_OPROM_NO_SPACE);
            break;
        case EFI_EVENT_LOG_TYPE_PASSWORD_INVALIDED:
            ClassSubClassToken = STRING_TOKEN (STR_EVENT_LOG_TYPE_PASSWORD_INVALIDED);
            break;
        //TH0053<<<
        default:
...
// Check the ErrorManagerSize for not to cross 16KB before setting it
// Here check for 16KB - 3*sizeof(ERROR_LIST). Because LINK should be 
