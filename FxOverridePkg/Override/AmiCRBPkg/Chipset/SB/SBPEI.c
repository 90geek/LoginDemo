...
#endif
                                        // <(P010609A)
EFI_GUID gPasswordJumperGuid = OEM_PASSWORD_JUMPER_GUID; //TH0025
//TH0089>>>>
#if  Thor_SUPPORT == 1
...
//    UINT32                      Addr;
    ...
    //TH0089>>>>
        ...
        UINT32                      SecAndSubBusNum = 0;
        UINT32                      VendId;
        UINT8                       FunNum = 0;
    	UINT32						Address = 0;	//TH0118
    	UINT32						Value = 0;		//TH0118
    #endif
    //TH0089<<<<
...
//TH0025>>>>
    Status = (*PeiServices)->CreateHob(PeiServices,
                                       ...
                                       sizeof(OEM_PASSWORD_JUMPER_HOB),
                                       &PasswordJumperHob);
    ASSERT_PEI_ERROR(PeiServices, Status);
    if (EFI_ERROR(Status))
        return Status;

    PasswordJumperHob->EfiHobGuidType.Name = gPasswordJumperGuid;
    
    TCO2Status = READ_IO16_TCO(ICH_IOREG_TCO2_STS);
    if (TCO2Status & 0x0001)
    	PasswordJumperHob->ClearPassword = 0x01;
    else 
    	PasswordJumperHob->ClearPassword = 0x00;
//TH0025<<<<
//TH0089>>>>
#if  Thor_SUPPORT == 1
    // Check Intel I350 of On Board LAN port Count.
    SecAndSubBusNum = READ_MEM32 (SB_PCIE_CFG_ADDRESS(0, 0x1C, 4, 0x18));
    
    Mmio16 (SB_PCIE_CFG_ADDRESS(0, 0x1C, 4, 0), 0x19) = 0x2520;

    VendId = READ_MEM32 (PCIE_CFG_ADDR(0x20, 0, FunNum, 0));
    while(VendId == 0x15218086){
    	FunNum++;
    	VendId = READ_MEM32 (PCIE_CFG_ADDR(0x20, 0, FunNum, 0));
    }
    Mmio32 (SB_PCIE_CFG_ADDRESS(0, 0x1C, 4, 0), 0x18) = SecAndSubBusNum;

    //Create Hob
    Status = (*PeiServices)->CreateHob(PeiServices,
                                       EFI_HOB_TYPE_GUID_EXTENSION,
                                       sizeof(OEM_BOARD_SKU_HOB),
                                       &ThorOemBoardSkuHob);
    ASSERT_PEI_ERROR(PeiServices, Status);
    if (EFI_ERROR(Status))
        return Status;

    ThorOemBoardSkuHob->EfiHobGuidType.Name = gOemBoardSkuGuid;
    ThorOemBoardSkuHob->OnBoardI350PortCount = FunNum;
    
    PEI_TRACE((-1, PeiServices,"\nOEM Debug: SBPEI.c ThorOemBoardSkuHob->OnBoardI350PortCount = %d \n",ThorOemBoardSkuHob->OnBoardI350PortCount));    
    ...
    if (Value & BIT29)
    	ThorOemBoardSkuHob->LanOcpCardPresent = FALSE;
    else
    	ThorOemBoardSkuHob->LanOcpCardPresent = TRUE;
    
    PEI_TRACE((-1, PeiServices,"\nOEM Debug: SBPEI.c ThorOemBoardSkuHob->LanOcpCardPresent = %d \n",ThorOemBoardSkuHob->LanOcpCardPresent));    
    //TH0118<<<
#endif
//TH0089<<<<
/*
