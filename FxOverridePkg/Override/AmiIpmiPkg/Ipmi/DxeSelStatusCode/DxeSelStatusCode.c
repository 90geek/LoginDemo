  ...
  VOID )
{
    ...
    if ( SelInfo.OperationSupport & RESERVE_SEL_COMMAND_SUPPORTED_MASK ) {
        Status = gIpmiTransport->SendIpmiCommand (
                    ...
                    NULL,
                    0,
//TH0032                     (UINT8 *) &ReserveId,
                    (UINT8 *) ReserveId, //TH0032 
                    (UINT8 *) &ReserveIdSize);
    ...
    ReportStatusCode (
        ...
        (EFI_SOFTWARE_EFI_BMC | EFI_SW_EC_EVENT_LOG_FULL) );
...
//LLY011031-                (UINT8 *) &RecordId,
                (UINT8 *) RecordId, //LLY011031+
                &ResponseDataSize);
#endif

    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " gIpmiTransport-> SendIpmiCommand Status: %r \n", Status));
    return Status;
}
...
/**
    ...
    This is the standard EFI driver point. This function initializes the private
    data required for creating SEL Driver.It gets the status codes records from
    data hub and logs to SEL.
  ...
  IN EFI_SYSTEM_TABLE   *SystemTable )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " InitializeDxeSel Entered.........\n"));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " ServerMgmtConfiguration.SelLogging: %X \n", ServerMgmtConfiguration.SelLogging));
    ...
    if ( (ServerMgmtConfiguration.SelErase == ERASE_NEXT_BOOT) || (ServerMgmtConfiguration.SelErase == ERASE_EVERY_BOOT) ) {
        ...
        if (ServerMgmtConfiguration.SelErase == ERASE_NEXT_BOOT) {
            ...
            SERVER_IPMI_DEBUG ((EFI_D_LOAD, " ServerMgmtConfiguration.SelErase: %X \n", ServerMgmtConfiguration.SelErase));
    ...
    Status = gBS->InstallProtocolInterface (
                ...
                &gSelProtocol);
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " (gEfiSelStatusCodeProtocolGuid Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    ...
    if (EFI_ERROR (Status)) {
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Create event is failing, So returning without registering DxeSelStatusCodeReportWorker handler\n"));
        return Status;
    }
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " gEfiRscHandlerProtocolGuid Status: %r \n", Status));
    ASSERT_EFI_ERROR (Status);
    if (!EFI_ERROR (Status)) {
        Status = RscHandlerProtocol->Register (DxeSelStatusCodeReportWorker, TPL_HIGH_LEVEL);
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, " RscHandlerProtocol->Register Status: %r \n", Status));
    }

    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " InitializeDxeSel Exiting.........\n"));
