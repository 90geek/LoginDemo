...
)
{
    ...
    *NumBytes = 0;
    ...
    if (Unicode < 0x0080) {   //7 bits
        ...
        *NumBytes         = 3;
        //TH0034 >>>>
        if ((((Unicode & 0xff00) == 0x2500) || ((Unicode & 0xff00) == 0x2100))) {
            for(i = 0; i < sizeof(gUnicodeToAscii)/sizeof(UNICODE_TO_ASCII); i++) {
                if (gUnicodeToAscii[i].Unicode == Unicode)
                    Utf8Char->UTF8_1 = gUnicodeToAscii[i].Ascii;
            }
            *NumBytes = 1;
        }
        //TH0034 <<<<
        ...
        switch (TerminalDev->TerminalType) {
            case ANSI:
                if (c) {
                    if (!isValidAnsi(c))
                    {
                        //TH0034 c = TranslateToAnsi(c);
                        c = TranslateToAscii(c); //TH0034
                        TransalteAscii=TRUE;
                        if (!c) {c = '.'; Warning = TRUE;}
                    }
...
//<AMI_PHDR_START>   
//   
// Procedure: TerminalSendCursorPositionEscapeSequence   
//   
...
//<AMI_PHDR_END>   
//**********************************************************************   
EFI_STATUS TerminalSendCursorPositionEscapeSequence(   
    IN EFI_SIMPLE_TEXT_OUTPUT_PROTOCOL  *This   
)   
{   
    EFI_STATUS Status = EFI_SUCCESS;   
    ...
    Column = (UINT8)This->Mode->CursorColumn;   
    Row = (UINT8)This->Mode->CursorRow;   
...
{
//TH0168>>>
    if (This->Mode->CursorColumn == 0)  //Only set cursor once on the line begining.
        TerminalSendCursorPositionEscapeSequence(This);
//TH0168<<<
