...
VOID InitTerminalStrings(IN EFI_HII_HANDLE HiiHandle, IN UINT16 Class)
{
    ...
    UINTN SioSerialPortsLocationVarSize = sizeof(SIO_SERIAL_PORTS_LOCATION_VAR);
...
//TH0108<<<	
        ...
        //TH0108>>>
                ...
                if (!EFI_ERROR(Status)) {
                    ...
                    if (IoRead8(0x3F8)!= 0xFF) StrCat(Str, L" (COM1)");
                    if (IoRead8(0x2F8)!= 0xFF) StrCat(Str, L" (COM2)");
#else
                    if (SioSerialPortsLocationVar.PortUid[i] == 0) 
                        StrCat(Str, L" (Physical)");
                    if (SioSerialPortsLocationVar.PortUid[i] == 1) 
                        StrCat(Str, L" (SOL)");
