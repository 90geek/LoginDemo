...
BOOLEAN NvProgram(IN VOID *Address, UINTN Size, VOID *Data)
{
    ...
    //TH0053  if (!Runtime && !Status) ERROR_CODE(DXE_FLASH_UPDATE_FAILED,EFI_ERROR_MAJOR);
    //TH0053>>>
    if (!Runtime && !Status){
    	 ...
    	 UINT64                    	RecordId;
    	 UINT8 Record = EFI_EVENT_LOG_TYPE_OEM_FLASH_UPDATE_FAILED;   //Example for Error Type. You could modify for need.
...
//TH0085>>>
#if defined(NEVER_RESTRICT_GLOBAL_VAR) && NEVER_RESTRICT_GLOBAL_VAR   
    return FALSE;
#endif 
//TH0085<<<
...
//TH0179>>>
	//
	//Log:Firmware (BIOS) ROM corruption Detected into SEL.  
	//
    ERROR_CODE(ROM_CORRUPTION_FAILURE,EFI_ERROR_MAJOR);    
//TH0179<<<
...
)
{
    ...
    //CheckStore(FALSE); // AptioV Server Override: To optimize the GetVariable performance by eliminating SPI access
#if NV_CACHE_SUPPORT
    {
        UINTN VariableNameSize;
        NVRAM_STORE_INFO *Info;
