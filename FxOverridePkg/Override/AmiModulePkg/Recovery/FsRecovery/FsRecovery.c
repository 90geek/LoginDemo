...
#include <PPI/FwVersion.h> //TH0042
...
//TH0042 >>>>
#if Foxconn_SUPPORT
#define $FID_Signature 0x44494624

EFI_STATUS OEM_CheckROMID(
    IN VOID     *Buffer,
    IN UINTN    Filesize )
{
    UINTN         Fv_BB_Size = 0x10000 * FV_BB_BLOCKS;
    EFI_STATUS    Status = EFI_NOT_FOUND;
    UINT8         *pFile;
    UINT8         *pROM;
    UINTN         i, j;

    pROM = (UINT8*)0xffffffff;
    for ( i = 0, pROM -= 4; i < Fv_BB_Size; i++, pROM-- ) {
        if ( (*(UINT32*)pROM) == $FID_Signature ) {
        	pFile = (UINT8*)Buffer;
            pFile += Filesize;
            for ( j = 0, pFile -= 4; j < Fv_BB_Size; j++, pFile-- ) {
            	if ( (*(UINT32*)pFile) == $FID_Signature ) {
            		for ( i = 0; i < 5; i++ ) {
            			if ( ((FW_VERSION*)pROM)->BiosTag[i] != ((FW_VERSION*)pFile)->BiosTag[i] )
            				return Status;
                    }
                    Status = EFI_SUCCESS;
                }
...
#if Foxconn_SUPPORT
          					{
          						Status = OEM_CheckROMID(Buffer, Size);
          						if (!EFI_ERROR(Status))
          							return EFI_SUCCESS;
          					}
#else
...
)
{
    ...
    *NumberOfFiles = 0;     // no files found yet
    ...
    while ( Count < RootSize ) {
        ...
        //
        FileNameSize = 0;
        while(( FileNameSize < Root->LengthOfFileId ) && (DirFileName[FileNameSize] != ';')) 
            FileNameSize++;
