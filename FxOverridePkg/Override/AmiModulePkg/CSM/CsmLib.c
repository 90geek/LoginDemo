...
{
//ZA0042 >>>>
#if defined(CHANGE_ONBOARD_HDD_FIRST) && CHANGE_ONBOARD_HDD_FIRST == 1  //TH0185
	  UINT16    i;
	  BBS_TABLE *TempBbsEntry;
#endif //TH0185
//ZA0042 <<<<
...
//ZA0042 >>>>
#if defined(CHANGE_ONBOARD_HDD_FIRST) && CHANGE_ONBOARD_HDD_FIRST == 1  //TH0185
  	pBS->AllocatePool(EfiBootServicesData, (gCoreBiosInfo->BbsEntriesNo)*sizeof(BBS_TABLE), &TempBbsEntry);
  	pBS->CopyMem(TempBbsEntry,gCoreBiosInfo->BbsTable, (gCoreBiosInfo->BbsEntriesNo)*sizeof(BBS_TABLE));
  	if((BbsEntry->DeviceType == BBS_HARDDISK)&&
  		(BbsEntry->Bus == 0x00) && ((BbsEntry->Device == 0x11) || (BbsEntry->Device == 0x1F)) && ((BbsEntry->Function == 0x04) || (BbsEntry->Function == 0x02))) {
  		for(i=0; i < gCoreBiosInfo->BbsEntriesNo; i++) {
  			if((gCoreBiosInfo->BbsTable[i].DeviceType == BBS_HARDDISK) &&
  				!((gCoreBiosInfo->BbsTable[i].Bus == 0x00) &&
  				 ((gCoreBiosInfo->BbsTable[i].Device == 0x11) || (gCoreBiosInfo->BbsTable[i].Device == 0x1F)) &&
  				 ((gCoreBiosInfo->BbsTable[i].Function == 0x04) || (gCoreBiosInfo->BbsTable[i].Function == 0x02))))
  				break;
  		}
  		gCoreBiosInfo->BbsTable[i] = *BbsEntry;
  		for(; i < gCoreBiosInfo->BbsEntriesNo; i++) {
  			gCoreBiosInfo->BbsTable[i+1] = TempBbsEntry[i];
  		}
