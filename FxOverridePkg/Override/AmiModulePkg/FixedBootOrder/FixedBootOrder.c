...
#endif
{
    ...
    //ZA0030  KeyValue = (KeyValue & 0x7f) + 1;
    KeyValue = (KeyValue & 0xff)+1; //ZA0030
...
/**
    Update the variable in Std default

    @param VarNames - Variable Nmae to be updated
    @param pFixedBootGroup - Variable Data to be updated
...
)
{
    ...
    TRACE((FBO_TRACE_LEVEL, "[FixedBootOrder.c] GetEfiVariable(%S) %r\n", VarNames, Status));
    if(EFI_ERROR(Status))
        return;
    ...
    pNVAR = (NVAR*)NvFindVariable(L"FixedBoot",&FboGroupFormFormSetGuid,&NameSize,&NvStore);
    if(pNVAR)
    {
...
)
{
    ...
    //Set all types device count and set variable FixedBootGroup.
    {
        ...
        pRS->SetVariable(
            L"FixedBootGroup",
            &gSetupGuid,
            //TH0186 BOOT_NV_ATTRIBUTES,
            VARIABLE_ATTRIBUTES,//TH0186
