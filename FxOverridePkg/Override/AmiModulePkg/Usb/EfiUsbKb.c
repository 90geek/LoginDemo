...
)
{
	...
	ProcessMultiLanguage(&NewEfiKey);
...
//TH0179>>>					
    	//
    	//Report USB KBC Controller error code.
    	//
    if(EFI_ERROR(Status)){
    	DEVICE_ERROR_CODE(DXE_KEYBOARD_CONTROLLER_ERROR, EFI_ERROR_MAJOR, Controller); 
    }  
//TH0179<<<
...
)
{
	...
	DestroyKeyBuffer(&UsbKbd->DevInfo->UsbKeyBuffer);
	DestroyKeyBuffer(&UsbKbd->DevInfo->KeyCodeBuffer);
...
)
{
    ...
    return ReadEfiKey(UsbKbd, KeyData, sizeof(AMI_EFI_KEY_DATA));
}
