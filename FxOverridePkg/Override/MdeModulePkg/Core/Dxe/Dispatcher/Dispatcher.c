...
#include "DxeMain.h"
#include <AmiCompatibilityPkg\Include\AmiStatusCodes.h>  //TH0179++
//
...
//TH0179>>>  
        if(Status == EFI_VOLUME_CORRUPTED){
			...
			//
        	REPORT_STATUS_CODE (EFI_ERROR_CODE | EFI_ERROR_MAJOR,ROM_CORRUPTION_FAILURE);       	
        }        
//TH0179<<<        
      } else {
        ...
        if (!EFI_ERROR (GetNextFileStatus)) {
          if (Type == EFI_FV_FILETYPE_DXE_CORE) {
            ...
            if (EFI_ERROR (Status)) {
              ...
              //
            	//TH0179--   CoreProcessFvImageFile (Fv, FvHandle, &NameGuid);
                  Status = CoreProcessFvImageFile (Fv, FvHandle, &NameGuid); //TH0179++
//TH0179>>>  
				  if(Status == EFI_VOLUME_CORRUPTED){
					//
					//Log:Firmware (BIOS) ROM corruption Detected into SEL.  
