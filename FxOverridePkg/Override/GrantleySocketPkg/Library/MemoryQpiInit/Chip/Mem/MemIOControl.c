  ...
  )
{
  ...
  if ((host->var.common.bootMode == S3Resume) || (host->var.mem.subBootMode == WarmBootFast)) {
    ...
    if ((host->nvram.mem.dramType == SPD_TYPE_DDR4) && (host->nvram.mem.dimmTypePresent == RDIMM)) {
      //STS001005- if (!(IsBDXEP(host) && CheckSteppingGreaterThan( host, CPU_BDX, A0_REV_BDX))) {  // WA reversed for BDX ML B0 s5003486--remove this line
     ...
     //STS001005- } s5003486--remove this line
    }  //if DDR4 RDIMM
