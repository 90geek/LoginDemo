...
#include <PfatDefinitions.h> //AptioV server override
#include <Cpu\CpuRegs.h> //AptioV server override
...
//
BOOLEAN     PfatEnabled = FALSE; //AptioV server override
  ...
  )
{
  ...
  } else { //AptioV server override end
  ...
  for (ArrayNumber = 0; ArrayNumber < CapsuleCount ; ArrayNumber++) {
    ...
    if ((CapsuleHeader->Flags & CAPSULE_FLAGS_PERSIST_ACROSS_RESET) == 0) {
      if (EfiAtRuntime ()) { 
          Status = EFI_UNSUPPORTED; //AptioV server override
      } else {
        Status = ProcessCapsuleImage(CapsuleHeader);
      }
      if (EFI_ERROR(Status)) {
  ...
  )
{
  ...
  if (!PfatEnabled) { //AptioV server override end
  for (ArrayNumber = 0; ArrayNumber < CapsuleCount; ArrayNumber++) {
    CapsuleHeader = CapsuleHeaderArray[ArrayNumber];
    //
    // A capsule which has the CAPSULE_FLAGS_POPULATE_SYSTEM_TABLE flag must have
