...
BOOLEAN IsMouseOnSoftkbd(VOID); //EIP-177886 Start
VOID 	GetCoordinates(INT32 *x, INT32 *y, INT32 *z); //EIP-177886 End
//<AMI_PHDR_START>
//----------------------------------------------------------------------------
...
EFI_STATUS PopupStringHandleAction( POPUP_STRING_DATA *popupstring, ACTION_DATA *Data)
{
	...
	if ( Data->Input.Type == ACTION_TYPE_MOUSE )
	{
		...
		if((Data->Input.Data.MouseInfo.Top >= (UINT32)popupstring->Top) && (Data->Input.Data.MouseInfo.Top < (UINT32)(popupstring->Top+popupstring->Height)) &&
			(Data->Input.Data.MouseInfo.Left >= (UINT32)popupstring->Left) && (Data->Input.Data.MouseInfo.Left < (UINT32)(popupstring->Left+popupstring->Width)))//EIP-123727
...
UINTN _PopupStringEditGetValue( POPUPEDIT_DATA *popupedit )
{
    ...
    while ( res == (UINT8)-1 )
	{
		...
		if (gST->ConOut)
		{
			//EIP-177886 Stop mouse when mouse pointer is over edit textbox cursor
			if ( 	
				...
				( ((MousePointerY/HiiGetGlyphHeight ()) <= (UINT32)gST->ConOut->Mode->CursorRow+1) && ((MousePointerY/HiiGetGlyphHeight ()) >= (UINT32)gST->ConOut->Mode->CursorRow-1) )
				)
				MouseStop ();
			else
			{
		...
		//EIP-177886 Do mouserefresh only when mouse pointer is out side softkbd
		if ( !IsMouseOnSoftkbd() )
			MouseRefresh();
