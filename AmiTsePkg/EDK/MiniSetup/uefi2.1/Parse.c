...
extern BOOLEAN HideDynamicFormsets (EFI_GUID *FormSetGuid ); //EIP-95647, 128758
...
//			DefaultSize = 0;			//If defaultstore 2 is present then changing to 0 will mal function so commented.
    ...
    *OptimalValue = Optimal;
    ...
    // EIP-111918: QuestionId is set unused. Adding fake usage for GCC build. 
    if (QuestionId) {};
  ...
  while(i < pkgHdr->Length)
	{
		...
		case EFI_IFR_FORM_SET_OP:
			...
			if ( HideDynamicFormsets(&formSet->Guid) ) //EIP-95647, 128758 Suppressing formsets mentioned in elink AMITSE_SUPPRESS_DYNAMIC_FORMSET_LIST
			{
				FormSetTitle = 0;
...
*/
        ...
        case EFI_IFR_FORM_MAP_OP: 
			...
			for (jIndex = 0; jIndex < gGuidDumpCount; jIndex ++)			//EIP64253 Check for offline vfr's
			{
				...
				{
					status = EFI_ABORTED;		//EIP180008. If not returning status as aborted here then it will increment dynamic page count so	
