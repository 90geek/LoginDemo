...
#endif
//TH0015 >>>>
#if defined EventLogsSetupPage_SUPPORT && EventLogsSetupPage_SUPPORT == 1
...
//TH0025>>>>
#include "AmiCRBPkg\Board\SB\SB.h"
#include "AmiCRBPkg\Chipset\SB\SBCspLib.h"
//TH0025<<<<
...
/*++
Routine Description:
  This function checks to see if the password needs to be cleared
  via a HW jumper.
...
Returns:
  TRUE - The password clear jumper is asserted
  FALSE - The password clear jumper is not set
--*/
BOOLEAN	TestPasswordClear_ByHob ()
{
	 OEM_PASSWORD_JUMPER_HOB	*PasswordJumperHob;
	 EFI_STATUS           		Status;
	 EFI_GUID 					gHobListGuid = HOB_LIST_GUID;
	 EFI_GUID 					gPasswordJumperGuid = OEM_PASSWORD_JUMPER_GUID;
	 
	 PasswordJumperHob = (OEM_PASSWORD_JUMPER_HOB*)GetEfiConfigurationTable(gST,&gHobListGuid);
	 
	 if (PasswordJumperHob != NULL){
	 	Status = FindNextHobByGuid((EFI_GUID*)&gPasswordJumperGuid, &PasswordJumperHob);
	    if (!EFI_ERROR(Status)){
	    	if (PasswordJumperHob->ClearPassword == 0x1)
	            return TRUE;
	    }
	 }
...
BOOLEAN	TestPasswordClear_ByRegister ()
{
	UINT16	Data = 0;

	Data = READ_IO16_TCO(ICH_IOREG_TCO2_STS);
...
BOOLEAN	TestPasswordClear ()
{
	return	TestPasswordClear_ByHob () || TestPasswordClear_ByRegister ();
}

VOID ClearSetupPassword () 
{ 
    AMITSESETUP AmiTseData0;
    UINTN	VariableSize = sizeof(AMITSESETUP);
	EFI_GUID gAmiTseSetupGuid = AMITSESETUP_GUID;

    gRT->GetVariable (  L"AMITSESetup", \
    	                &gAmiTseSetupGuid, \
    	                NULL, \
                        &VariableSize, \
                        &AmiTseData0 );
						
    MemSet(&AmiTseData0.AdminPassword, 0x14*sizeof(CHAR16), 0);
    MemSet(&AmiTseData0.UserPassword, 0x14*sizeof(CHAR16), 0);
...
//TH0120>>>
EFI_STATUS   
WriteStringToTerminals(   
    CHAR16 *Message,   
    INTN Column,   
    INTN Row,   
    BOOLEAN PreventedCusPos)
{   
    ...
    UINTN		Index, CurrentCol, CurrentRow;
    ...
    for (Index = 0; Index < HandleCount; Index++) {   
        ...
        if ((pDp->Header.Type == MESSAGING_DEVICE_PATH) &&   
                (pDp->Header.SubType == MSG_VENDOR_DP)) {   

            if ((guidcmp(&pDp->Guid,&gEfiVT100Guid) != 0) &&   
                    (guidcmp(&pDp->Guid,&gEfiVT100PlusGuid) != 0) &&   
                    (guidcmp(&pDp->Guid,&gEfiVTUTF8Guid) != 0) &&   
                    (guidcmp(&pDp->Guid,&gEfiPcAnsiGuid) != 0))   
            {
                continue;
            }
            ...
            CurrentCol = pConOut->Mode->CursorColumn;
            CurrentRow = pConOut->Mode->CursorRow;

            pConOut->SetCursorPosition(pConOut, Column, Row);   
            pConOut->OutputString(pConOut, Message);   
            	
            if (PreventedCusPos) {   
            	pConOut->SetCursorPosition(pConOut, CurrentCol, CurrentRow);   
            }   
        }   
    }   
...
//TH0120>>>>
//TH0192>>>
#if defined HOTKEY_PROMPT_MESSAGE_FONT_COLOR && HOTKEY_PROMPT_MESSAGE_FONT_COLOR == 1
	 ...
	 //TH0120>>>
		{
			if (gQuietBoot) {   
			  	PosX = 10;   
			  	PosY -= 60;  
		  		PostManagerDisplayQuietBootMessage (text, PosX, PosY, CA_AttributeCustomised,   
				                                     Foreground, Background); 
		  		WriteStringToTerminals(text,1,1,TRUE);
			} else
		//TH0120<<<
			PostManagerDisplayPostMessage(text);
	  }	//TH0120
...
//TH0014 >>>>
		...
		text = HiiGetString( gHiiHandle, STRING_TOKEN(STR_F7_ENTER_POPUP) );
		if ( text != NULL )
		//TH0120>>>
		{
    		if (gQuietBoot) {   
    		  	...
    		  	PosY += 20;  
	   	 		PostManagerDisplayQuietBootMessage (text, PosX, PosY, CA_AttributeCustomised,   
    			                                     Foreground, Background); 
	  			WriteStringToTerminals(text,1,2,TRUE);
		 	} else
		 //TH0120<<<
			PostManagerDisplayPostMessage(text);
		}	//TH0120
...
#if	SETUP_OEM_KEY1_ENABLE
{
	...
	if(!EFI_ERROR(Status) && SetupData.PxeOpRom){
		text = HiiGetString( gHiiHandle, STRING_TOKEN(STR_PXE_BOOT_MENU) );
		if ( text != NULL )
			//TH0120>>>
		{
			if (gQuietBoot) {   
				...
				PostManagerDisplayQuietBootMessage (text, PosX, PosY, CA_AttributeCustomised,   
				                                     Foreground, Background); 
				WriteStringToTerminals(text,1,0,TRUE);	
			} else
		//TH0120<<<
        PostManagerDisplayPostMessage(text);
     }	//TH0120
...
VOID ProcessEnterSetup(VOID)
{
    ...
    if ( gPasswordType == AMI_PASSWORD_NONE )
    {
    		...
    		if (TestPasswordClear()) {     
    			PasswordInstalled = 0;
    			WRITE_IO16_TCO(ICH_IOREG_TCO2_STS, 0x0001);
    		} else
    		//TH0025<<<<
      	PasswordInstalled = PasswordCheckInstalled();
...
//TH0106>>>>
#if	SETUP_OEM_KEY1_ENABLE
void PrintEnterPXEMessage()
{
   	if(gEnterPXE != TRUE) // To print the "Entering Popup" message only once
	{						
		CHAR16 *text = NULL;
		EFI_GUID	SetupGuid = SETUP_GUID;
		EFI_STATUS	Status;
		...
		if(!EFI_ERROR(Status) && SetupData.PxeOpRom){
			text = HiiGetString( gHiiHandle, STRING_TOKEN(STR_ACK_PXE_BOOT_MENU) );
			if ( text != NULL )
			PostManagerDisplayPostMessage(text);
		    MemFreePointer( (VOID **)&text );
			gEnterPXE = TRUE;
...
//TH0120<<<
	...
	do
	{
		...
		{
			//TH0120>>>
#if defined(NetBootHotKey_SUPPORT) && (NetBootHotKey_SUPPORT)   
            if ((SETUP_OEM_KEY1_UNICODE == AmiKey.Key.UnicodeChar) && (SETUP_OEM_KEY1_SCAN == AmiKey.Key.ScanCode)) {   
            ...
            if(((SETUP_ENTRY_UNICODE == AmiKey.Key.UnicodeChar) && (SETUP_ENTRY_SCAN == AmiKey.Key.ScanCode))   
            ...
            if((POPUP_MENU_ENTRY_UNICODE == AmiKey.Key.UnicodeChar) && (POPUP_MENU_ENTRY_SCAN == AmiKey.Key.ScanCode)) {   
            	CheckandDeactivateSoftkbd();
            	if (text == NULL)
            		text = HiiGetString (gHiiHandle, STRING_TOKEN(STR_ACK_BBS_POPUP));   
            }   
            ...
            if(gQuietBoot) {
            	if (text != NULL) {
            		...
            		if ( !EFI_ERROR(GetScreenResolution(&PosX, &PosY)) ) {
            			PosX = 10;   
            			PosY -= 20;
						PostManagerDisplayQuietBootMessage (text, PosX, PosY, CA_AttributeCustomised,   
															Foreground, Background);  
						WriteStringToTerminals(text,1,3,TRUE);
