...
$(BUILD_DIR)/AMITSEStrTokens.h : 
	$(ECHO) \
"#include <$(BUILD_DIR)$(PATH_SLASH)$(PLATFORM_NAME)$(PATH_SLASH)$(TARGET)_$(TOOL_CHAIN_TAG)$(PATH_SLASH)$(AMITSE_TARGET_ARCH)/Build/AMITSE/DEBUG/AMITSEStrDefs.h>$(EOL)\
">$(BUILD_DIR)/AMITSEStrTokens.h
...
HPKTOOL_COMMAND_LINE+=-r -dn$(NVRAM_RECORD_CHECKSUM_SUPPORT) -de$(FLASH_ERASE_POLARITY) -vs$(EFI_SPECIFICATION_VERSION) -dl$(DEFAULT_LANGUAGE) -dp$(SETUP_ORPHAN_PAGES_AS_ROOT_PAGE)\
...
## when token is enabled RT will add to form (RT+BS+NV) 0x07
#############################################################################
...
$(EOL)END { for (item in IfrList) print \"IFR_DIR_LIST+= -i\"item }" > $(BUILD_DIR)/IfrListScript.txt
else
APTIO_MAKE:=$(MAKE)
TAB := \t
...
../$(FXPKG_DIR)/FXUpdateInfoToBMC.c #GCH001002+$(EOL)\
...
[Packages]$(EOL)\
  ...
  AmiTsePkg\AmiTsePrivatePkg.dec   #TH0120$(EOL)\
