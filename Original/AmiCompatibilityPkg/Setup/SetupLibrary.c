  ...
  )
{
    ...
    if(CallbackFunction != NULL)
    {
            ...
            NextLink = TempLink, TempLink = (AMI_CALLBACK_LIST_ENTRY*)GetNextNode(&AmiCallbackList, &(NextLink->Link))) 
        {
            ...
            {
                RemoveEntryList((LIST_ENTRY*)&(TempLink->Link));
  ...
  )
{
    ...
    if(ExtractConfigFunction != NULL)
    {
        ...
        for(NextLink = (AMI_EXTRACT_CONFIG_ENTRY*)GetFirstNode(&AmiExtractConfigList), 
            TempLink = (AMI_EXTRACT_CONFIG_ENTRY*)GetNextNode(&AmiExtractConfigList, &(NextLink->Link)); 
            !IsNull(&AmiExtractConfigList, &(NextLink->Link)); 
            NextLink = TempLink, TempLink = (AMI_EXTRACT_CONFIG_ENTRY*)GetNextNode(&AmiExtractConfigList, &(NextLink->Link))) 
  ...
  )
{
    ...
    if(AccessRouteConfigFunction != NULL)
    {
        ...
        for(NextLink = (AMI_ROUTE_CONFIG_ENTRY*)GetFirstNode(&AmiRouteConfigList), 
            TempLink = (AMI_ROUTE_CONFIG_ENTRY*)GetNextNode(&AmiRouteConfigList, &(NextLink->Link)); 
            !IsNull(&AmiRouteConfigList, &(NextLink->Link)); 
            NextLink = TempLink, TempLink = (AMI_ROUTE_CONFIG_ENTRY*)GetNextNode(&AmiRouteConfigList, &(NextLink->Link))) 
        {
