    ...
    if(!ResourcesLoaded)
    {
        ...
        if(!EFI_ERROR(Status))
        {
            ...
            for(NextLink = (AMI_STRING_INITIALIZATION_ENTRY*)GetFirstNode(&AmiStringInitializationList), 
                TempLink = (AMI_STRING_INITIALIZATION_ENTRY*)GetNextNode(&AmiStringInitializationList, &(NextLink->Link)); 
                !IsNull(&AmiStringInitializationList, &(NextLink->Link)); 
                NextLink = TempLink, TempLink = (AMI_STRING_INITIALIZATION_ENTRY*)GetNextNode(&AmiStringInitializationList, &(NextLink->Link))) 
