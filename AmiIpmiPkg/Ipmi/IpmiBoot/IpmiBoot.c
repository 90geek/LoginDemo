...
#define IsUsbDeviceClassCode(Option) (((Option)->BbsEntry->Class == \
    PCI_CL_SER_BUS) && ((Option)->BbsEntry->SubClass == \
    PCI_CL_SER_BUS_SCL_USB))
#define isUsbCompatibleDevice(Option,DevType) \
    (((((Option)->BbsEntry->InitPerReserved) >> 24)& 0XFF) == DevType)
#define isDevicePathMatch(pDp, DpType, DpSubType) \
    ( (pDp)->Type==DpType && (pDp)->SubType==DpSubType )
#define isAcpiDeviePath(pDp)  ( (pDp)->Header.Type==ACPI_DEVICE_PATH && \
    (pDp)->Header.SubType==ACPI_DP && \
    (pDp)->HID==EISA_PNP_ID(0x0604))
#define isBbsDeviePath(pDp, BbsDevType)  \
    ( (pDp)->Header.Type==BBS_DEVICE_PATH && \
    (pDp)->Header.SubType==BBS_BBS_DP && \
    (pDp)->DeviceType==BbsDevType)
  ...
  IN IPMI_BOOT_INFO_ACKNOWLEDGE        *Param4 )
{
    ...
    ResponseDataSize = sizeof (IPMI_BOOT_INFO_ACKNOWLEDGE_RESPONSE);
  ...
  IN UINT16 BootOptionNumber )
{
    ...
    if ( Buffer != NULL ) {
        Status = gBS->AllocatePool(
                     ...
                     (VOID **)&MatchedBootOptionDetails);

        if ( !EFI_ERROR(Status) ) {
            EfiZeroMem( MatchedBootOptionDetails, \
                (NvramOptionSize + BOOT_OPTION_NUMBER_SIZE) );
            ...
            if ( !EFI_ERROR(Status) ) {
                Status = pBS->FreePool (MatchedBootOptionDetails);
            }
        }
    } //if (Buffer != NULL)
}
...
/**
    This function exclude the end node alone from the given device path and
    returns the address of newly created device path.

    @param pDp The device path to be copied.

    @retval VOID * Address of the newly created device path. Returns NULL if there
                   is not enough memory or pDp is NULL.
**/
  ...
  IN  EFI_DEVICE_PATH_PROTOCOL    *pDp )
{
    ...
    EfiAllocatePool (EfiBootServicesData, \
        len + sizeof(EFI_DEVICE_PATH_PROTOCOL), pdp);
    ASSERT(pdp);
    EfiCopyMem (pdp, pDp, len);
  ...
  VOID )
{
    ...
    SERVER_IPMI_TRACE ((-1, "IPMI Boot: Entered SaveIPMISetBootOrder.\n"));
    //
    ...
    Buffer = GetVariableAndSize (
                 ...
                 &BufferSize);
    if (Buffer != NULL) {
        SERVER_IPMI_TRACE ((-1, "IPMI Boot: Saving IPMI Set BootOrder.\n"));
        //
    ...
    SERVER_IPMI_TRACE ((-1, "IPMI Boot: Exiting SaveIPMISetBootOrder.\n"));
    return;
}
  ...
  VOID )
{
    ...
    SERVER_IPMI_TRACE ((-1, "IPMI Boot: Entered ClearIPMIBootVaraible.\n"));
    //
    ...
    if (IPMIBootOrder != NULL) {
        ...
        SERVER_IPMI_TRACE ((-1, "IPMI Boot: Exiting as IPMI Set Boot Order not available.\n"));
        return;
    }
    //
    ...
    if (CurrBootOrder != NULL) {
        while (Index < (CurrBootOrderSize / sizeof (UINT16))) {
            if (IPMIBootOrder[Index] != CurrBootOrder[Index]) {
                ...
                SERVER_IPMI_TRACE ((-1, "IPMI Boot: Boot Order Changed by user.\n"));
            }
            Index = Index + 1;
        }
    }
    ...
    if(!BootOrderChange) {
        if (CheckDevicePriorityChange ()) {
            BootOrderChange = TRUE;
            SERVER_IPMI_TRACE ((-1, "IPMI Boot: Legacy device priority Changed by user.\n"));
        }
    }
#endif //#if CSM_SUPPORT
    if (BootOrderChange) {
    ...
    SERVER_IPMI_TRACE ((-1, "IPMI Boot: Exiting ClearIPMIBootVaraible.\n"));
  ...
  VOID )
{
    ...
    SERVER_IPMI_TRACE ((-1, "IPMI Boot: Entered RestoreBootorder.\n"));
    //
    ...
    Buffer = GetVariableAndSize (
                 ...
                 &BootOptionSize);
    if ( Buffer != NULL ) {
        SERVER_IPMI_TRACE ((-1, "IPMI Boot: IpmiMatchBootOptionData Available.\n"));
        ...
        //
        if ( !EFI_ERROR(Status) ) {
            SERVER_IPMI_TRACE ((-1, "IPMI Boot: Clearing IpmiMatchBootOptionData.\n"));
            Status = gRT->SetVariable (
                         L"IpmiMatchBootOptionData",
                         &gEfiIpmiBootGuid,
                         VAR_FLAG,
    ...
    if ( BootOrder != NULL ) {
        SERVER_IPMI_TRACE ((-1, "IPMI Boot: Restoring Original boot order.\n"));
        //
    ...
    SERVER_IPMI_TRACE ((-1, "IPMI Boot: Exiting RestoreBootorder.\n"));
  ...
  VOID )
{
    ...
    SERVER_IPMI_TRACE ((-1, "\n IPMI Boot: Entered SetIpmiBootOptionPriorities. \n"));
    ...
    Status = gBS->LocateProtocol (
                 ...
                 NULL,
                 (VOID **)&gIpmiTransport );
    SERVER_IPMI_TRACE((-1, "IPMI Boot: LocateProtocol gEfiDxeIpmiTransportProtocolGuid. Status: %r\n", Status));
    if ( EFI_ERROR (Status) ) {
        TRACE((TRACE_ALWAYS, "SetIpmiBootOptionPriorities: Error in locating \
...
*/
    ...
    SERVER_IPMI_TRACE ((-1, " IPMI Boot: Exiting SetIpmiBootOptionPriorities. \n"));
    return;
}
  ...
  IN  UINT16                    DpSubType )
{
    ...
    EFI_DEVICE_PATH_PROTOCOL    *TempOptionDp;
            ...
            if ( gUefiBoot && (Option->BbsEntry == NULL) && \
                ...
                if ( (DpType == ACPI_DEVICE_PATH) && (DpSubType == ACPI_DP) ) {
                    if (isAcpiDeviePath( (ACPI_HID_DEVICE_PATH*)OptionDp )) {
                        ...
                        DevicePath = DPGetLastNode (OptionDp);
                        if ( isDevicePathMatch(DevicePath, \
                    ...
                    EFI_DEVICE_PATH_PROTOCOL    *DevicePath;
                    DevicePath = DPGetLastNode (OptionDp);
                    if ( isDevicePathMatch(DevicePath, DpType, DpSubType)) {
                        //
                        // Cut the end node from the device path to find out
                        // whether the device is USB compatible or Non USB 
                        // compatible device.
                        //
                        TempOptionDp = EfiCutDevicePathEndNode (OptionDp);
                        DevicePath = DPGetLastNode (TempOptionDp);
                        //
                        ...
                        //
                        if (isDevicePathMatch (DevicePath, MESSAGING_DEVICE_PATH, \
                            MSG_USB_DP)) {
                            UsbDeviceCount = UsbDeviceCount + 1;
                        } else {
                            continue;
...
#if BBS_USB_DEVICE_TYPE_SUPPORT == 0
        ...
        FOR_EACH_BOOT_OPTION (BootOptionList, Link, Option){
            ...
            if (gUefiBoot && (Option->BbsEntry == NULL) && \
                ...
                if ((DpType == ACPI_DEVICE_PATH) && (DpSubType == ACPI_DP)) {
                    ...
                    // Check for other UEFI devices
                    //
                    } else if ( isDevicePathMatch (DevicePath, DpType, DpSubType)) {
                        //
                        // Cut the end node from the device path to find out
                        // whether the device is USB compatible or Non USB
                        // compatible device
                    ...
                    //
                        ...
                        if (isDevicePathMatch (DevicePath,MEDIA_DEVICE_PATH,MEDIA_FILEPATH_DP)){    // check if FILEPATH_DEVICE_PATH?
                            DevicePath = DPGetLastNode (TempOptionDp);
                            if(isDevicePathMatch(DevicePath, DpType, DpSubType)){
                                //
                                // Cut the end node from the device path to find out
        ...
        if ( !gUefiBoot && BootNumber ) {
            FOR_EACH_BOOT_OPTION (BootOptionList, Link, Option){
                if ( BootNumber == Option->BootOptionNumber ) {
                    Option->Priority = HIGHEST_BOOT_OPTION_PRIORITY;
                    //TH0079>>>
                    // If more than one BootOption have the same BootOptionNumber,
                    // set all these BootOptions' Tag to HIGHEST_BOOT_OPTION_TAG.
                    if ( !Option->GroupHeader )
                        Option->Tag = HIGHEST_BOOT_OPTION_TAG;
					          //TH0079<<<
                }
  ...
  )
{
    ...
    SERVER_IPMI_TRACE ((-1, "IPMI Boot: gIpmiForceBootDevice = %d gPersistentBoot = %d gUefiBoot = %d gDeviceInstanceSelector = %d\n", \
  ...
  OUT GET_BOOT_FLAG_RESPONSE        *BootFlagsResponse )
{
    ...
    SERVER_IPMI_TRACE ((-1, " IpmiGetBootFlags: Entered......\n"));

    //
    // Wait until Set In Progress field is cleared
    //
    Status = IpmiWaitSetInProgressClear ();
    SERVER_IPMI_TRACE ((-1, " IpmiWaitSetInProgressClear: Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                 ...
                 &ResponseSize );
    SERVER_IPMI_TRACE ((-1, " IpmiGetBootFlags: EFI_CHASSIS_GET_SYSTEM_BOOT_OPTIONS Status: %r \n", Status));
    SERVER_IPMI_TRACE ((-1, " IpmiGetBootFlags: Exiting........ Status: %r \n", Status));
  ...
  VOID )
{
    ...
    //
    do {
        ResponseSize = sizeof (GET_SET_IN_PROGRESS_RESPONSE);
        Status = gIpmiTransport->SendIpmiCommand (
                     gIpmiTransport,
                     EFI_SM_NETFN_CHASSIS,
                     0,
  ...
  IN UINT8      BootInitAckData )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                 ...
                 (UINT8*) &SetRequest,
                 sizeof (SET_BOOT_INFO_ACK_REQUEST),
                 (UINT8*) &SetResponse,
                 &ResponseSize );
