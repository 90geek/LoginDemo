...
#include <Library/ReportStatusCodeLib.h>	//TH0066
...
#define EFI_SW_DXEM_OS_WATCHDOG_TIMEOUT         (EFI_SUBCLASS_SPECIFIC | 0x00000004)	//TH0066
...
BOOLEAN                             gTimerCtlFlag = FALSE;  //AptioV server override - Preventing gFlag redefinition error.
  ...
  OUT EFI_FRB_STAT                     *FrbStatData )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " GetFrbStatus entered........FrbType: %x \n", FrbType));
    if (FrbStatData == NULL) {
    ...
    FrbTimerDataSize = sizeof(EFI_BMC_GET_WATCHDOG_TIMER_STRUCTURE);
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                &FrbTimerDataSize);
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " EFI_APP_GET_WATCHDOG_TIMER Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    //
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " GetFrbStatus Exiting........FrbStatData->FrbEnabled: %x FrbStatData->FrbFailed: %x \n", FrbStatData->FrbEnabled, FrbStatData->FrbFailed));
  ...
  IN UINTN                            *Timeout )
{
   ...
   SERVER_IPMI_DEBUG ((EFI_D_LOAD, " SetFrbTimerValue entered........FrbType: %x *Timeout: %x \n", FrbType, *Timeout));
   ...
   SERVER_IPMI_DEBUG ((EFI_D_LOAD, " SetFrbTimerValue Exiting........\n"));
  ...
  IN FRB_SYSTEM_TIMEOUT_INTERRUPT       FrbTimeoutInterrupt )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " SetFrbPreTimeOutPolicy entered........FrbType: %x FrbTimeoutInterrupt: %x \n", FrbType, FrbTimeoutInterrupt));
  ...
  IN FRB_SYSTEM_BOOT_POLICY           FrbBootPolicy )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " SetFrbBootPolicy entered........FrbType: %x FrbBootPolicy: %x \n", FrbType, FrbBootPolicy));
  ...
  IN BOOLEAN                            ClearFrbGlobalStatus )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " EnableFrb  entered........FrbType: %x\n", FrbType));
    ...
    if (FrbType == EfiFrb2) {
        ...
        if (HobList.Header->HobType != EFI_HOB_TYPE_HANDOFF) {
            SERVER_IPMI_DEBUG ((EFI_D_LOAD, "(DxeFrb) Hand off HOB missing!\n"));
            return EFI_NOT_FOUND;
        }
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8*)&GetWatchDogTimerData,
                &FrbTimerDataSize );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "EnableFrb() : EFI_APP_GET_WATCHDOG_TIMER Status : %r\n", Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
    //
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                NULL,
                &ResponseDataSize);
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " EFI_APP_SET_WATCHDOG_TIMER Status: %r \n", Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
    //
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                NULL,
                &ResponseDataSize);
    SERVER_IPMI_DEBUG ((EFI_D_INFO, " EnableFrb() : EFI_APP_RESET_WATCHDOG_TIMER Status: %r \n", Status));
  ...
  IN EFI_FRB_TYPE                     FrbType )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " DisableFrb  entered........FrbType: %x \n", FrbType));
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8*)&GetWatchDogTimerData,
                &FrbTimerDataSize );
    SERVER_IPMI_DEBUG ((EFI_D_INFO, "DisableFrb() : EFI_APP_GET_WATCHDOG_TIMER Status: %r \n", Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
    ...
    if ( !GetWatchDogTimerData.TimerUse.TimerRunning ) {
        DEBUG ((EFI_D_INFO, "DisableFrb() : EfiFrb2 already disabled  \n"));
        return EFI_ABORTED;
    }
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                NULL,
                &ResponseDataSize);
    SERVER_IPMI_DEBUG ((EFI_D_INFO, " DisableFrb() : EFI_APP_SET_WATCHDOG_TIMER Status: %r \n", Status));
    return Status;
}
  ...
  IN VOID             *Context )
{
    ...
    Status = gBS->LocateProtocol(
                    ...
                    NULL,
                    &legacyBootProtocol );
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "LocateProtocol gAmiLegacyBootProtocolGuid Status = %r\n", Status));
    ...
    if( !(EFI_ERROR(Status)) ) {
      if (gServerMgmtConfiguration.Frb2Enable) {
          Status = DisableFrb (
                    gFrbProtocol,
                    EfiFrb2 );
          DEBUG ((EFI_D_LOAD, "EfiFrb2 disabled Status = %r\n", Status));
      }

      if (gServerMgmtConfiguration.OsBootWdtEnable) {
          Status = EnableOsWdtTimer ();
          DEBUG ((EFI_D_LOAD, "EfiOsBootWdt Enabled Status = %r\n", Status));
      }
    }
}
...
VOID ) 
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " Status(Oswdt expiry function) = %x\n", Status));
    return Status;
}
  ...
  IN EFI_SYSTEM_TABLE     *SystemTable )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " InitializeDxeFrb Entered........\n"));
    ...
    // Log frb failure.
    //
    if (!EFI_ERROR(Status) && GetWatchDogTimerData.TimerUseExpirationFlagsClear.OsLoadExpirationFlag) {
		SERVER_IPMI_DEBUG ((EFI_D_LOAD, " EfiOsBootWdt flag expired..........Reporting it....\n"));
		ReportStatusCode (
			EFI_ERROR_CODE | EFI_ERROR_MINOR,
		(EFI_SW_DXEM_OS_WATCHDOG_TIMEOUT | EFI_SOFTWARE_DXE_CORE) );
    }
    //TH0066<<<

    gFrbProtocol = (EFI_SM_FRB_PROTOCOL *)AllocateZeroPool (sizeof (EFI_SM_FRB_PROTOCOL));
    if (gFrbProtocol == NULL) {
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Failed to allocate memory for gFrbProtocol! \n"));
    ...
    Status = gBS->InstallProtocolInterface (
                ...
                gFrbProtocol );
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " gEfiSgFrbProtocolGuid Install protocol : Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gRT->GetVariable Status %r \n", Status));
    if(EFI_ERROR(Status)) {
  ...
  IN VOID             *Context )
{
    ...
    gTimerCtlFlag = TRUE;  //AptioV server override - Preventing gFlag redefinition error.
    if ( gServerMgmtConfiguration.Frb2Enable ) {
         Status = DisableFrb (
                   gFrbProtocol,
                   EfiFrb2 );
         DEBUG ((EFI_D_LOAD, "EfiFrb2 disabled Before setup Status = %r\n", Status));
      ...
      if ( gServerMgmtConfiguration.OsBootWdtEnable ) {
           ...
           SERVER_IPMI_DEBUG ((EFI_D_INFO, "gTimerCtlFlag = %x\n", gTimerCtlFlag));  //AptioV server override - Preventing gFlag redefinition error.
      }
    }
}
  ...
  IN VOID             *Context )
{
    ...
    if (!FrbDisabled) {
        ...
        if (!EFI_ERROR(Status)) {
            DEBUG ((EFI_D_LOAD, "EfiFrb2 disabled Before OPRom Status = %r\n", Status));
            FrbDisabled = TRUE;
            return;
        }
    }
    if (FrbDisabled) {
        ...
        if (!EFI_ERROR(Status)) {
            FrbDisabled = FALSE;
            DEBUG ((EFI_D_LOAD, "EfiFrb2 Enabled after OPRom Status = %r\n", Status));
        }
    }
}
  ...
  IN VOID             *Context )
{
    ...
    Status = EnableFrb (
               ...
               FALSE,
               FALSE);
    DEBUG ((EFI_D_LOAD, "EfiFrb2 Enabled after Giving Password= %r\n", Status));
}
  ...
  IN VOID             *Context )
{
    ...
    Status = EnableFrb (
               ...
               FALSE,
               FALSE);
    DEBUG ((EFI_D_LOAD, "EfiFrb2 Enabled after Prompt TimeOut Ends= %r\n", Status));
}
...
DisableEnableOsWdtTimer (
  VOID ) {
    ...
    if (gServerMgmtConfiguration.OsBootWdtEnable) {
        ...
        if (gTimerCtlFlag == TRUE) {  //AptioV server override - Preventing gFlag redefinition error.
...
ClearFlag (
  ...
  IN VOID             *Context) {
    ...
    gTimerCtlFlag = FALSE;  //AptioV server override - Preventing gFlag redefinition error.
