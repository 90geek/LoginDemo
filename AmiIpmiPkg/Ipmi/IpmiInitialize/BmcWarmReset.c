 ...
 IN  UINT16             Key )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Key Value in BmcWarmReset callback function: %x\n", Key));
    ...
    Status = gBS->LocateProtocol (
                ...
                NULL,
                (VOID **)&AmiPostMgr);
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gAmiPostManagerProtocolGuid locateProtocol Status: %r\n", Status));
    ...
    if((!EFI_ERROR(OutputStrStatus)) && (!EFI_ERROR(CharPtrStatus))) {
        Status = AmiPostMgr->DisplayMsgBox (
                    ...
                    MSGBOX_TYPE_YESNO,
                    &MsgBoxSel );
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "DisplayMsgBox return Status: %r\n", Status));
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "MsgBoxSel: %x\n", MsgBoxSel));
    }
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " SendWarmResetCommandToBMC Status: %r \n", Status));
    if(EFI_ERROR(Status)) {
        BufferLength = STRING_BUFFER_LENGTH;
