  ...
  IN CONST EFI_PEI_READ_ONLY_VARIABLE2_PPI *ReadVariablePpi )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Entering IsIpmiCmosClear\n "));
    Size = sizeof (UINT8);
    Status = ReadVariablePpi->GetVariable (
                                    ...
                                    &Size,
                                    &Value );
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "ReadVariablePpi->GetVariable Status %r  \n", Status));
    if ( EFI_ERROR (Status)) {
        return FALSE;
    }
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Exiting IsIpmiCmosClear\n "));
    return TRUE;
}
