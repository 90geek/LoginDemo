  ...
  IN UINT8         DeviceState )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                EFI_SM_NETFN_APP,
                BMC_LUN,
                EFI_APP_SET_ACPI_POWERSTATE,
                //TH0032 (UINT8 *) &AcpiPowerState,
				        (UINT8 *) AcpiPowerState, //TH0032
  ...
  IN EFI_SYSTEM_TABLE     *SystemTable )
{
    ...
    for( Index=BMC_ACPI_SWSMI; Index<BMC_ACPI_SWSMI_MAX; Index++) {
        ...
        Status = SwDispatch->Register (
                    ...
                    &SwHandle);
        if (EFI_ERROR (Status)) {
            DEBUG ((EFI_D_LOAD, "Couldn't register the BMC ACPIh SW SMI %d handler.Status: %r\n",Index,Status));
            return Status;
        }
    }
    return EFI_SUCCESS;
