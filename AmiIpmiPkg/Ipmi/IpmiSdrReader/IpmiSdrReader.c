...
{
    EFI_STATUS                          Status;
    EFI_IPMI_SDR_REPOSITORY_INFO        SdrInfo;
    UINT8                               SdrInfoSize = sizeof (EFI_IPMI_SDR_REPOSITORY_INFO);
    UINT16                              *SdrVersionNo = NULL;
    UINTN                               SizeofBuffer;

    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " GetSdrVerNum entered... \n"));
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                                ...
                                (UINT8 *) &SdrInfo,
                                (UINT8 *) &SdrInfoSize );
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " EFI_STORAGE_GET_SDR_REPOSITORY_INFO Status: %r\n", Status ));
    if ( EFI_ERROR (Status) ) {
        return Status;
    }
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "SdrVersionNo Address : %x\n", SdrVersionNo));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "SdrVersionNo: %S \n", SdrVersionNo));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " GetSdrVerNum Exiting... \n"));
    return EFI_SUCCESS;
}
