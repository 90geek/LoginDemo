  ...
  IN VOID                      *Ppi )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "GetBootMode Status: %r BootMode: %x \n", Status, BootMode));
  ...
  IN  CONST EFI_PEI_SERVICES       **PeiServices )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " FrbEnabled: %x \n", FrbEnabled));
    ...
    if (FrbEnabled) {
        ...
        Status = IpmiTransportPpi->SendIpmiCommand (
                    ...
                    NULL,
                    &ResponseDataSize );
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, " EFI_APP_RESET_WATCHDOG_TIMER Status: %r \n", Status));
