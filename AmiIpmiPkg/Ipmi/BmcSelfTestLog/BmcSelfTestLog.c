...
//TH0109>>>>
    ...
    UINT8       BmcMinorFirmwareRev;
    UINT32      BmcAuxFirmwareRev;
#endif
//TH0109<<<<
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "BmcSelfTestLogStrings: Class ID:  %x\n", Class));
    //
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gRT->GetVariable ServerSetup status %r \n", Status));
    ...
    Status = gRT->GetVariable (
                    ...
                    &Size,
                    &gBstLog );
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gRT->GetVariable BstLogVar status %r \n", Status));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gBstLog.logSize:  %x \n", gBstLog.logSize));
    ...
    if ( (!EFI_ERROR (Status)) && BmcSelfTestProtocol->IsGetDeviceIdCommandPassed ) {
        ...
        InitString (
                ...
                L"%d",
                BmcSelfTestProtocol->ControllerInfo.DeviceId);
        ...
        InitString (
                ...
                L"%x",
                BmcSelfTestProtocol->ControllerInfo.DeviceRevision);
...
//TH0109>>>>
#if defined Thor_SUPPORT && Thor_SUPPORT == 1
//ZYY006013-        BmcMinorFirmwareRev = ((BmcSelfTestProtocol->ControllerInfo.MinorFirmwareRev & 0xF0) * 10) + \
//ZYY006013- 		                  (BmcSelfTestProtocol->ControllerInfo.MinorFirmwareRev & 0x0F);
        BmcMinorFirmwareRev = BmcSelfTestProtocol->ControllerInfo.MinorFirmwareRev; //ZYY006013+
 		BmcAuxFirmwareRev = ((BmcSelfTestProtocol->ControllerInfo.AuxFirmwareRevInfo & 0xFF) << 24) + \
 		               (((BmcSelfTestProtocol->ControllerInfo.AuxFirmwareRevInfo >> 8) & 0xFF) << 16) + \
 		               (((BmcSelfTestProtocol->ControllerInfo.AuxFirmwareRevInfo >> 16) & 0xFF) << 8) + \
 		               ((BmcSelfTestProtocol->ControllerInfo.AuxFirmwareRevInfo >> 24) & 0xFF);
...
//ZYY006013- 		        L"%d.%d.%08X",
 		        L"%d.%X.%08X", //ZYY006013+
 		        BmcSelfTestProtocol->ControllerInfo.MajorFirmwareRev, BmcMinorFirmwareRev,BmcAuxFirmwareRev);
#else
//TH0109<<<<
        InitString (
                HiiHandle,
                STRING_TOKEN(STR_IPMI_BMC_FIRMWARE_REVISION_VAL),
                L"%x.%x",
                BmcSelfTestProtocol->ControllerInfo.MajorFirmwareRev, BmcSelfTestProtocol->ControllerInfo.MinorFirmwareRev);
#endif	//TH0109
        //
        // IPMI Version
        //
        InitString (
                ...
                L"%x.%x",
                ((BmcSelfTestProtocol->ControllerInfo.SpecificationVersion)& 0x0F), ((BmcSelfTestProtocol->ControllerInfo.SpecificationVersion) >> 0x04));
    ...
    if (EFI_ERROR (Status)) {
        ...
        if (LogIndex < BST_LOG_SIZE) {
            ...
            UpdateTime (LogIndex);
#endif
            SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gBstLog.log[%x]:  %x \n", LogIndex, gBstLog.log[LogIndex]));
            LogIndex = LogIndex + 1;
        }
    } else {
        //
    ...
    Status = gRT->SetVariable (
                ...
                &gBstLog );
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gRT->SetVariable BstLogVar status %r \n", Status));
