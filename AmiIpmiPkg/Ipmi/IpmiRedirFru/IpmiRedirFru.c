...
{
    EFI_IPMI_FRU_GLOBAL          *FruPrivate = NULL;
    FRU_READ_COMMAND              FruReadCommandData = {0};
...
{
    EFI_IPMI_FRU_GLOBAL          *FruPrivate = NULL;
    FRU_WRITE_COMMAND             FruWriteCommandData;
    UINT8                         Count;
    UINT8                         ResponseData[20];
    UINT8                         ResponseDataSize;
    UINT16                        PointerOffset = 0;
  ...
  IN EFI_SYSTEM_TABLE       *SystemTable )
{
    ...
    EFI_IPMI_GET_SDR                    GetSdrCommandData = {0};
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " InitializeSmRedirFruLayer entered... \n"));
    ...
    if (gIpmiFruGlobal == NULL) {
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Failed to allocate memory for gIpmiFruGlobal! \n"));
    ...
    if ( EFI_ERROR (Status) ) {
        ...
        //
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, " Error in locating IPMI transport protocol so publish Dummy records...\n" ));
    ...
    if ( EFI_ERROR (Status) ) {
        ...
        //
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, " Get Device id command is not successful so publish Dummy records...\n" ));
        DummyFruRecords = TRUE;
        goto GenerateFruRecords;
    }

    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "ControllerInfo.FruInventorySupport %X\n", ControllerInfo.FruInventorySupport));
    if (ControllerInfo.FruInventorySupport) {
        ...
        ZeroMem (&gIpmiFruGlobal->FruDeviceInfo[0].FruDevice, sizeof (EFI_FRU_DATA_INFO));
        gIpmiFruGlobal->FruDeviceInfo[0].FruDevice.LogicalFruDevice = TRUE;
        gIpmiFruGlobal->NumSlots++;

        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "*( (UINT16*) &ControllerInfo.ManufacturerId[0] ) %X\n", *( (UINT16*) &ControllerInfo.ManufacturerId[0] ) ));
        // 
        // Verify the manufacturer ID and proceed....
        //
        if ( ( *( (UINT16*) &ControllerInfo.ManufacturerId[0] ) == IANA_PEN_IPMI_FORUM ) || \
           ( *( (UINT16*) &ControllerInfo.ManufacturerId[0] ) == IANA_PEN_INTEL ) || \
           ( *( (UINT16*) &ControllerInfo.ManufacturerId[0] ) == OEM_MANF_ID ) ) {
            //
            ...
            Status = gIpmiTransport->SendIpmiCommand (
                        ...
                        (UINT8 *) &ReserveId,
                        (UINT8 *) &ReserveIdSize );
            SERVER_IPMI_DEBUG ((EFI_D_LOAD, " EFI_STORAGE_RESERVE_SDR_REPOSITORY Status: %r ReserveId: %x\n", Status, ReserveId ));
            ...
            GetSdrCommandData.BytesToRead = 0x10; // Bytes to read
            ...
            while ( GetSdrCommandData.RecordId != 0xFFFF ) {
                ...
                SERVER_IPMI_DEBUG ((EFI_D_LOAD, " GetSdrCommandData.RecordId: %x\n", GetSdrCommandData.RecordId ));
                //
                ...
                Status = gIpmiTransport->SendIpmiCommand (
                            ...
                            //TH0032 (UINT8 *) &GetSdrResponseData,
                            (UINT8 *) GetSdrResponseData, //TH0032
                            (UINT8 *) &GetSdrResponseDataSize );
                SERVER_IPMI_DEBUG ((EFI_D_LOAD, " EFI_STORAGE_GET_SDR Status: %r RecordType: %x GetSdrResponseData.RecordId: %x\n", Status, ((EFI_IPMI_SDR_RECORD_STRUCT_HEADER*)GetSdrResponseData)->RecordType, ((EFI_IPMI_SDR_RECORD_STRUCT_HEADER*)GetSdrResponseData)->RecordId ));

                if ( EFI_ERROR (Status) ) {
                    //
                    // FRU data is not available from the BMC. As such we need to publish Dummy records.
                    //
                    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "  FRU data is not available from the BMC so publish Dummy records...\n" ));
                    DummyFruRecords = TRUE;
                    goto GenerateFruRecords;
                }
                //
                ...
                //  
                 if ( ((EFI_IPMI_SDR_RECORD_STRUCT_HEADER*)GetSdrResponseData)->RecordType == 0x11 ) {
                    Index = GetEmptyFruSlot ();
                    if (Index >= MAX_FRU_SLOT) {
                        DEBUG ((EFI_D_LOAD, "Increase the number of FRU Slots!!!\n"));
                        break;
                    }
                    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Type 0x11 Record Found: FruDeviceId:  %x\n",  gIpmiFruGlobal->FruDeviceInfo[Index].FruDevice.FruDeviceId ));
                    ...
                    CopyMem (
                        ...
                        (UINT8*)GetSdrResponseData + sizeof (EFI_IPMI_SDR_RECORD_STRUCT_HEADER),
                        sizeof (EFI_FRU_DATA_INFO)
                        );    
                }
                //
                // Update the RecordId for the GET SDR command
                //
                if ( GetSdrCommandData.RecordId == ((EFI_IPMI_SDR_RECORD_STRUCT_HEADER*)GetSdrResponseData)->NextRecordId ) {
                    break;
                } else {
                    GetSdrCommandData.RecordId = ((EFI_IPMI_SDR_RECORD_STRUCT_HEADER*)GetSdrResponseData)->NextRecordId; // Record ID
                }
            ...
            //
            SERVER_IPMI_DEBUG ((EFI_D_LOAD, " Manufacturer Id is not supported so publish Dummy records...\n" ));
            Status = EFI_UNSUPPORTED;
            DummyFruRecords = TRUE;
            goto GenerateFruRecords;
        }
    } else { //if (ControllerInfo.FruInventorySupport)
        //
        // FruInventory is not supported. So generate dummy FRU records.
        //
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, " FruInventory is not supported so publish Dummy records...\n" ));
        DummyFruRecords = TRUE;
        goto GenerateFruRecords;
    }
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, " InstallProtocolInterface gEfiRedirFruProtocolGuid Status: %r \n", Status));
