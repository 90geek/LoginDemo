...
#TH0001 >>>>
ifeq ($(Foxconn_SUPPORT),1)

ifeq ("$(FITC_NM_FILE_NAME)", "")
	@copy $(OUTPUT_DIR)\ROM\OutImage.bin $(BIOS_TAG).BIN	
else
	@copy $(OUTPUT_DIR)\ROM\OutImage_NM.bin $(BIOS_TAG).BIN
	@copy $(OUTPUT_DIR)\ROM\OutImage.bin $(BIOS_TAG)_SiEn.BIN
endif	
ifeq ($(SecureMod_SUPPORT),1)
    ifeq ($(wildcard $(FWpriv)), $(FWpriv))
	@$(ECHO) $(BIOS_TAG).BIN and $(FWCAPSULE_FILE_NAME) binaries created.
    else
	@$(ECHO) $(BIOS_TAG).BIN $(BIOS_TAG)_SiEn.BIN and $(BIOS_TAG).ROM binaries created.
    endif
else
	@$(ECHO) $(BIOS_TAG).BIN $(BIOS_TAG)_SiEn.BIN and $(BIOS_TAG).ROM binaries created.
endif
