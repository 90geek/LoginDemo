...
)
{
    ...
    Status = pRS->SetVariable(
                 L"CspResetType",
                 &VariableGuid,
                 EFI_VARIABLE_BOOTSERVICE_ACCESS, //AptioV Server Override: Remove NVRAM RT Attribute
                 DataSize,
                 &CurrentResetType);
...
)
{
    ...
    UINT32 LVL3_SEL;
    UINT32 INV1_SEL;
    //Before MRC init, global variable can not be changed   //TH0077
	  UINT32 RST1_SEL = 0;//TH0077
	  UINT32 RST2_SEL = 0;//TH0077
	  UINT32 RST3_SEL = 0;//TH0077
...
//        gRST3_SEL = IoRead32((UINT16)BaseAddress + ICH_GPIO_GP_RST_SEL3);
        RST1_SEL = IoRead32((UINT16)BaseAddress + ICH_GPIO_GP_RST_SEL1);
        RST2_SEL = IoRead32((UINT16)BaseAddress + ICH_GPIO_GP_RST_SEL2);
        RST3_SEL = IoRead32((UINT16)BaseAddress + ICH_GPIO_GP_RST_SEL3);
//TH0077<<<<
                                   // <(P091109D.1)
        ...
        for (i = 0; i < (Size - 1); i++)
        {
            ...
            if (pTable[i].CONFIG.OFFSET < 32)
            {
                ...
                //TH0077  gRST1_SEL = (gRST1_SEL & ~(1 << Buffer.OFFSET)) | (Buffer.RST << Buffer.OFFSET); // (P091109D.1)
                RST1_SEL = (RST1_SEL & ~(1 << Buffer.OFFSET)) | (Buffer.RST << Buffer.OFFSET); //TH0077
            }
            else if ((pTable[i].CONFIG.OFFSET >= 32) && (pTable[i].CONFIG.OFFSET < 64))
            {
                ...
                //TH0077  gRST2_SEL = (gRST2_SEL & ~(1 << (Buffer.OFFSET - 32))) | (Buffer.RST << (Buffer.OFFSET - 32)); // (P091109D.1)
                RST2_SEL = (RST2_SEL & ~(1 << (Buffer.OFFSET - 32))) | (Buffer.RST << (Buffer.OFFSET - 32)); //TH0077
            }
            else
            {
                ...
                //TH0077  gRST3_SEL = (gRST3_SEL & ~(1 << (Buffer.OFFSET - 64))) | (Buffer.RST << (Buffer.OFFSET - 64)); // (P091109D.1)
                RST3_SEL = (RST3_SEL & ~(1 << (Buffer.OFFSET - 64))) | (Buffer.RST << (Buffer.OFFSET - 64)); //TH0077
            }
        }
...
//        IoWrite32((UINT16)BaseAddress + ICH_GPIO_GP_RST_SEL3, gRST3_SEL);
        IoWrite32((UINT16)BaseAddress + ICH_GPIO_GP_RST_SEL1, RST1_SEL);
        IoWrite32((UINT16)BaseAddress + ICH_GPIO_GP_RST_SEL2, RST2_SEL);
        IoWrite32((UINT16)BaseAddress + ICH_GPIO_GP_RST_SEL3, RST3_SEL);
//TH0077<<<<
                                        // <(P091109D.1)
    }
...
// #if defined(FtRecovery_SUPPORT) && (FtRecovery_SUPPORT == 1)  // AptioV Server Override - To avoid Build Error in SPIFlash.c on disabling FtRecovery_SUPPORT
//<AMI_PHDR_START>
//----------------------------------------------------------------------------
...
#endif
//#endif  // AptioV Server Override - To avoid Build Error in SPIFlash.c on disabling FtRecovery_SUPPORT
//*************************************************************************
//*************************************************************************
//**                                                                     **
//**        (C)Copyright 1985-2011, American Megatrends, Inc.            **
