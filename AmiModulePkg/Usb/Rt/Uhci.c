    ...
    UINT16      wLength)
{
	...
	DataToggle = UsbGetDataToggle(DevInfo, DevInfo->bIntEndpoint);

    //
    // Form device address and endpoint in proper order and bit position
    //
    dTemp = (UINT32)bEndp << 7;
    dTemp = (dTemp | (DevInfo->bDeviceAddress)) << 8; //[10:0] = Dev. Addr & Endpoint
                                                        //[18:8] = Dev. Addr & Endpoint
    //
	...
	dValue |= DevInfo->bIntEndpoint & BIT7 ? UHCI_TD_IN_PACKET : 
		UHCI_TD_OUT_PACKET;
    ...
    UsbUpdateDataToggle(DevInfo, DevInfo->bIntEndpoint, IntQh->DataToggle);
    ...
    DEV_INFO    *DevInfo)
{
	...
	PacketId = DevInfo->bIntEndpoint & BIT7 ? UHCI_TD_IN_PACKET : UHCI_TD_OUT_PACKET;
	...
	PollTd = UhciAllocGeneralTds(DevInfo->bDeviceAddress, LowSpeed, PacketId, 
					DevInfo->bIntEndpoint & 0xF, DevInfo->wIntMaxPkt, TRUE, 
...
{
    UHCI_DESC_PTRS      *UhicDescPtrs = HcStruc->stDescPtrs.fpUHCIDescPtrs;
    UHCI_QH *Qh =  UhicDescPtrs->RepeatQh;
