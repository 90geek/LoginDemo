...
UINT8
GetMassDeviceName(
    CHAR16              *DevName,  //AptioV server override - Fix for garbage under Mass storage devices name field
    UINT8               DevAddr,
    EFI_USB_PROTOCOL *UsbProtocol
)
{
    CHAR8   Data[48];
    UINT8   AsciiCodeData[48] = {0};  //AptioV server override - Fix for garbage under Mass storage devices name field
    UINT8   NextDev;

    // Get the name using USBMassAPIGetDeviceInformation
    NextDev = UsbProtocol->UsbGetNextMassDeviceName(Data, sizeof(Data), DevAddr);
  //AptioV server override start- Fix for garbage under Mass storage devices name field
    if (NextDev != 0xFF) {
        Sprintf((char*)AsciiCodeData, "%a", Data);
    }

    AsciiStrToUnicodeStr(AsciiCodeData, DevName);
  //AptioV server override end- Fix for garbage under Mass storage devices name field
    return NextDev;
}
...
)
{
    ...
    CHAR16  MassStr[MAX_DEVS_LINE_LENGTH];  //AptioV server override - Fix for garbage under Mass storage devices name field
    UINT8   NextDev;
    UINT16  MassDev[16] = {
