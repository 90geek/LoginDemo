...
//TH0167>>>   
#if defined(PATCH_FOR_BMC_USB_VM_NAME) && (PATCH_FOR_BMC_USB_VM_NAME == 1)   
UINT8   
IsSubString(   
        UINT8   *pString,   
        UINT8   *pSubString   
...
{   
    ...
    for (i = 0; pString[i] != 0; i++){   
        if (pString[i] == pSubString[0]){   
            pIndex = &pSubString[0];   
            for (j = i; pString[j] != 0; j++){   
                if (pString[j] != *pIndex) break;   
                pIndex++;   
                if (*pIndex == 0) break;   
            }   
            if (*pIndex == 0) {   
...
{   
    UINT8   i, Index = 0;   
    UINT8   *pKey;   
//  UINT8   Indedicator[] = "MP EMS Virtual Media";   
    UINT8   Indedicator1[] = "EMS";   
    UINT8   Indedicator2[] = "Virtual";   
    UINT8   Pattern[] = "Media";   
    UINT8   *Replace[4] = {"HDD  ", "ODD  ", "FDD  ", ""};   
  
    Index = IsSubString(pDeviceName, Indedicator1);   
    if (Index == -1) return;   
    Index = IsSubString(pDeviceName, Indedicator2);   
    if (Index == -1) return;   
       
    Index = IsSubString(pDeviceName, Pattern);   
    if (Index == -1) return;   
    ...
    for (i = 0; pKey[i] != 0; i++) {   
        pDeviceName[Index + i] = pKey[i];   
    }   
    return;   
}   
#endif //defined(PATCH_FOR_BMC_USB_VM_NAME) && (PATCH_FOR_BMC_USB_VM_NAME == 1)   
//TH0167<<<   
...
//TH0148>>>
    // Do not install USB FDD.
    if(DevInfo->bStorageType == USB_MASS_DEV_ARMD)
...
//TH0148<<<
    ...
    if (DevInfo->bLastStatus & USB_MASS_MEDIA_PRESENT) {
        MassDev->Media->LastBlock = DevInfo->MaxLba;   // LastBlock is 0-based
  ...
  )
{
    ...
    if ((Lba + (BufferSize / BlockSize)) > This->Media->LastBlock) {
        return EFI_INVALID_PARAMETER;
    }
...
)
{
    ...
    PatchForBmcVMDeviceString(DevInfo->DevNameString,DevInfo->bStorageType);//LLY011009+
