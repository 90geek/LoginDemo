  ...
  if((ProtocolCapability == NULL) || (This == NULL)){
    ...
    if(ProtocolCapability->Size < (sizeof(UINT8) + sizeof(AMI_TREE_VERSION) + sizeof(AMI_TREE_VERSION))){
      ...
      if (ProtocolCapability->Size < sizeof(AMI_TREE_BOOT_SERVICE_CAPABILITY)){
        ...
        if( TcgLog->HashAlg == AMI_TREE_BOOT_HASH_ALG_SHA1){
            ProtocolCapability->HashAlgorithmBitmap = AMI_TREE_BOOT_HASH_ALG_SHA1;
        }else if(TcgLog->HashAlg == AMI_TREE_BOOT_HASH_ALG_SHA256){
    ...
    IN  UINTN               HashAlgo)
{
    ...
    *(UINT32 *)Buffer = TPM_H2NL(1);
    ...
    if(HashAlgo==AMI_TREE_BOOT_HASH_ALG_SHA1){
        //Hash alg
	    *(UINT16 *)Buffer = TPM_H2NS(TPM2_ALG_SHA1);
        Buffer += sizeof(UINT16);
    ...
    if(HashAlgo==AMI_TREE_BOOT_HASH_ALG_SHA256){
	    //Hash alg
...
)
{
  ...
  static  UINTN             EventCount=0; 
  static  UINTN             HashAlgoInfoLogged = FALSE;
  ...
  if(NewEventHdr->EventType == EV_EFI_ACTION)
   ...
   if(ConfigFlags.HashPolicy == AMI_TREE_BOOT_HASH_ALG_SHA1 || EventCount == 0)
   {
      ...
      if(DataToHash != NULL && NewEventHdr->EventType != EV_NO_ACTION)
       {
          Status = SHA1HashAll( NULL, DataToHash, DataSize, (UINT8 *)&NewEventHdr->Digests.digests.sha1);        
          if(EFI_ERROR(Status))return Status;
       }
   }else if(ConfigFlags.HashPolicy == AMI_TREE_BOOT_HASH_ALG_SHA256)  //for Sha256
   {
       ...
       if(DataToHash != NULL && NewEventHdr->EventType != EV_NO_ACTION)
        {
    	   ...
    	   if(EFI_ERROR(Status)){
    	       DEBUG((-1, "SHA2HASHALL Failed %r \n", Status));
    	       return Status;
    	   }
           Status = SHA1HashAll( NULL, DataToHash, DataSize, (UINT8 *)&EventData.digests.digests[0].digest.sha1);
              if(EFI_ERROR(Status)){
                  DEBUG((-1, "SHA1HASHALL Failed %r \n", Status));
                  return Status;
           }
		   // we should copy the SHA2 digest , and later extend on PCR bank.
            CopyMem(
       ...
       {
           SetMem((UINT8 *)&EventData.digests.digests[1].digest.sha256, SHA256_DIGEST_SIZE, 0);
           SetMem((UINT8 *)&EventData.digests.digests[0].digest.sha1, SHA1_DIGEST_SIZE, 0);
       }
       
       if(DataToHash == NULL){

           CopyMem((VOID*)(&EventData.digests.digests[1].digest.sha256),
                   &NewEventHdr->Digests.digests.sha256,
                   SHA256_DIGEST_SIZE);
           CopyMem((VOID*)(&EventData.digests.digests[0].digest.sha1),
                   &NewEventHdr->Digests.digests.sha1,
                   SHA1_DIGEST_SIZE);
       }
...
EvNoAction:   
   ...
   DEBUG(( -1," LastEntry = %x \n", TcgLog->LastEntry));
     if(TcgLog->LastEntry == 0) return EFI_ABORTED;
    
     TcgLog->LastEventPtr = TcgLog->LastEntry;

     CopyMem((VOID*)(UINTN)TcgLog->LastEntry,
                                    NewEventHdr,
     ...
     DEBUG((-1, "first TempSize = %x \n"));
    
    if(ConfigFlags.HashPolicy == AMI_TREE_BOOT_HASH_ALG_SHA1 || HashAlgoInfoLogged == FALSE)
    {
            ...
            HashAlgoInfoLogged = TRUE;
            
    }else if(ConfigFlags.HashPolicy == AMI_TREE_BOOT_HASH_ALG_SHA256)
    {
            *(UINT32*)(TcgLog->LastEntry + TempSize) = EventData.digests.count;
    ...
    IN CONST EFI_PEI_SERVICES    **PeiServices)
{
    ...
    if(AutoSupportType())return EFI_SUCCESS;
    ...
    if(InterfaceSupported){
        ...
        if(!ErrorCrbInterface && !EFI_ERROR(Status)){
            ...
            TcgLog->TableSize = 0;
            TcgLog->HashAlg = ConfigFlags.HashPolicy;
