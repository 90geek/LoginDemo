...
#include "../../GrantleyPkg/Me/Include/Protocol/HeciSmm.h" // AptioV Server Override: EFI_SMM_HECI_PROTOCOL structure change conflict resolution
...
EFI_SMM_HECI_PROTOCOL            *SmmHeci = NULL; // AptioV Server Override: EFI_SMM_HECI_PROTOCOL structure change conflict resolution
...
EFI_STATUS HMRFPO_MERESET(VOID)
{
        ...
        Status = SmmHeci->SendMsg((UINT32*)&HmrfpoMeResetReq, sizeof(HmrfpoMeResetReq),
        ...
        MeFs1.DWord = GetHFS();//AptioV Grantley refresh override 
...
EFI_STATUS HMRFPO_ENABLE_MSG(VOID)
{
	...
	#if DISABLE_ME_WORKAROUND 
    	{
            ...
            if (EFI_ERROR(Status)) return Status;
	          ...
	          Status = SmmHeci->SendwACK((UINT32*)&HeciMsg.HmrfpoEnableReq,
...
EFI_STATUS HMRFPO_LOCK_MSG(VOID)
{
	    ...
	    // Clear Status
	      ...
	      Status = SmmHeci->SendwACK((UINT32*)&HeciMsg.HmrfpoLockReq,
    ...
    switch(StructPtr->bBlockType)
    {
        ...
        case ME_OPR_BLK:
            if((HFS & 0xF) != 2)
            {
                ...
                {
                    MEProcessHandleResult(BIT03 | BIT00,
        ...
        case DER_BLK:
                Status = GetRegionOffset(5, &Offset, &Length);
                if(Status == EFI_NOT_FOUND)
                    return EFI_UNSUPPORTED;
                *Step = 0;
...
)
{
    ...
    if(Step == 1)
    {
        ...
        if(EFI_ERROR(Status) || (HMRFPO_STATUS != 0))
        {
            MEProcessHandleResult(BIT03 | BIT02 | BIT00,
                      "Error when sending Enable Message to ME !!");
            return Status;
        }
