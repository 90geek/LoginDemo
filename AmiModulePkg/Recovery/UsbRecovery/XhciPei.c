...
/**
    ...
    @param FfsHeader  --  EFI_FFS_FILE_HEADER pointer
    @param PeiServices --  EFI_PEI_SERVICES pointer

    @retval EFI_STATUS (Return Value) = EFI_SUCCESS on successful 
    completion or valid EFI error code
...
)
{
         ...
         ControllerIndex++, XhciBaseAddress+=0x10000)
    {
        ...
        Usb3Hc->DbOffset = Usb3Hc->CapRegs->DbOff;      // Door bell offset
...
/**
    This function allocates XHCI scratch pad buffers.
...
)
{
    ...
    (**PeiServices).SetMem(ScratchBufEntry, (sizeof(UINT64) * NumBufs), 0);
    ...
    for (Count = 0; Count < NumBufs; Count++) {    
        ...
        ScratchBufEntry[Count] = (UINTN)TempPtr + (Usb3Hc->OpRegs->PageSize << 12) * Count;
    }
...
/**
    ...
    @param PeiServices -- PEI_SERVICES pointer
    @param Usb3Hc -- XHCI controller data structure pointer
    @param ControllerIndex -- 0-based index of the controller to be initialized
...
)
{
    ...
    if (Usb3Hc->OpRegs->UsbSts.Field.HcHalted == 0)
    {
        ...
        for (i = 0; i < PEI_XHCI_HALT_TIMEOUT_MS; i++) {
            XHCI_FIXED_DELAY_MS(Usb3Hc, 1);
...
/**
    ...
    @retval EFI_STATUS (Return Value)= EFI_SUCCESS on successful pre-configuration completion
...
)
{
    ...
    TRB_RING                      *XfrRing;
    UINT8                         Speed;
    static UINT16                 aMaxPacketSize[6] = {0, 8, 8, 64, 512, 512};
    EFI_STATUS                    Status;
    UINT8                         SlotId;
...
//    XHCI_FIXED_DELAY_MS(Usb3Hc, 1000);
    ...
    switch (DeviceSpeed) {
        ...
        case USB_SLOW_SPEED_DEVICE:
            Speed = XHCI_DEVSPEED_LOW;
            break;
        case USB_FULL_SPEED_DEVICE:
            Speed = XHCI_DEVSPEED_FULL;
            break;
    ...
    gSlotBeingConfigured = SlotId;  // Valid from now till SetAddress
...
/**
    ...
    @retval SlotId variable is updated
    @retval EFI_STATUS (Return Value)= EFI_SUCCESS if update is successful
...
/**
    ...
    @param Stalled EP data - SlotId and DCI
...
/**
    ...
    door bell. Alternatively end point is restarted using Configure Endpoint command.
...
/**
    ...
    @param  Device - Evaluated device context pointer
...
)
{
    ...
    // Prepare input context for EvaluateContext command
    (**PeiServices).SetMem((UINT8*)Usb3Hc->InputContext, XHCI_INPUT_CONTEXT_ENTRIES * Usb3Hc->ContextSize, 0);
...
/**
    This function initiates a USB control transfer and waits on it to 
    complete.

        
    @param PeiServices --  EFI_PEI_SERVICES pointer
    @param This --  PEI_USB_HOST_CONTROLLER_PPI pointer
    @param bDeviceAddress --  USB address of the device for which the control 
        transfer is to be issued
    @param DeviceType --  Not used
    @param MaximumPacketLength --  Maximum number of bytes that can be sent to or 
        received from the endpoint in a single data packet
    @param Request --  EFI_USB_DEVICE_REQUEST pointer
    @param TransferDirection --  Direction of transfer
    @param DataBuffer --  Pointer to source or destination buffer
    @param DataLength --  Length of buffer
    @param TimeOut --  Not used
    @param TransferResult --  Not used
...
/**
    This function initiates a USB bulk transfer and waits on it to 
    complete.

        
    @param PeiServices --  EFI_PEI_SERVICES pointer
    @param This --  PEI_USB_HOST_CONTROLLER_PPI pointer
    @param DeviceAddress --  USB address of the device for which the control 
        transfer is to be issued
    @param EndPointAddress --  Particular endpoint for the device
    @param MaximumPacketLength --  Maximum number of bytes that can be sent to or 
        received from the endpoint in a single data packet
    @param DataBuffer --  Pointer to source or destination buffer
    @param DataLength --  Length of buffer
    @param DataToggle --  Used to update the control/status DataToggle field
...
)
{
    ...
    *TransferResult = EFI_USB_NOERROR;
    ...
    // The outer loop repeats the transfer if the requested transfer size exceeds
...
)
{
    ...
    switch (PortFeature) {
        ...
        case EfiUsbPortConnectChange:
            PortSC->AllBits = (PortSC->AllBits & XHCI_PCS_PP) | XHCI_PCS_CSC | XHCI_PCS_PRC |XHCI_PCS_WRC;
            break;
...
/**
    This routine resets the Host Controller and its USB bus 
    ...
    @retval EFI_UNSUPPORTED if unknown attribute is supplied
...
/**
    ...
    @retval EFI_DEVICE_ERROR on execution failure, otherwise EFI_SUCCESS
    @retval Params pointer to the command specific data.

    @note  
  Caller is responsible for a data place holder.
...
/**
    ...
    @retval Pointer to the transfer ring
...
/**
    ...
    For SuperSpeed bulk and control end points, the Interval field shall not be
    ...
    For high-speed and SuperSpeed Interrupt and Isoch end points the bInterval
    field the Endpoint Descriptor is computed as 125æs * 2^(bInterval-1), where
    bInterval = 1 to 16, therefore Interval = bInterval - 1.

    For low-speed Interrupt and full-speed Interrupt and Isoch end points the
...
/**
    This function parses the device descriptor data and enables the end points
    ...
    @retval EFI_DEVICE_ERROR on error, EFI_SUCCESS on success
...
)
{
    ...
    for (CurPos = 0; CurPos < TotalLength; CurPos += EpDesc->Length) {
        ...
        // 1) For Isoch, Interrupt, or Bulk type end points the DCI is calculated
        ...
        // end points and 1 for IN end points.
        // 2) For Control type end points:
...
/**
    This function calculates and returns the pointer to a door bell for a
    given Slot.
    ...
    for(;;) {
        switch (CurPtr->CapId) {
            ...
            case XHCI_EXT_CAP_USB_DEBUG_PORT:
                ...
                PEI_TRACE((-1, PeiServices, "XHCI: USB Debug Capability Ptr %x\n", Usb3Hc->DbCapRegs));
                break;
        }
