...
/**
    ...
    @param PeiServices --  PEI Devices table pointer
    @param ControllerIndex --  Index of the controller in the global
        PCI_BUS_DEV_FUNCTION array

    @retval UINT32 (Return Value)= Base address for this controller's operational
        registers.
...
/**
    Check debug port of a controller is in use or not.   
    ...
    @note  EHCIDbg_PortUsed used to maintain the debug port usage of 
...
//
// Retrieve the debug port base offset address
//
    (*PeiServices)->PciCfg->Read( PeiServices, (*PeiServices)->PciCfg,
    ...
    DebugBase = DebugBase & 0x1FFF; // Bar number hard wired to 001b
...
)
{
    ...
    // for each controller.  Initialize the controller and setup
...
/**
    ...
    @param PeiServices  --  EFI_PEI_SERVICES pointer
    @param EhciDevPtr --  PEI_EHCI_DEV pointer
    @param Index --  Index of this controller in the global 
        PCI_BUS_DEV_FUNCTION array

    @retval VOID (Return Value)
...
)
{
    ...
    EhciHcOpReg->UsbSts.AllBits = 
           (EHCI_USB_INTERRUPT |        // Interrupt
            ...
            EHCI_FRAME_LIST_ROLLOVER |  // Frame List Roll over
...
/**
    ...
    @note  It is assumed that all necessary operational register data has been 
...
/**
    ...
    @param PortNumber --  Port number for which status is requested
    @param PortStatus --  EFI_USB_PORT_STATUS pointer's data is updated
...
/**
    ...
    @param PeiServices --  EFI_PEI_SERVICES pointer
    @param This --  PEI_USB_HOST_CONTROLLER_PPI pointer
    @param PortNumber --  Number of downstream ports
...
/**
    ...
    @param PeiServices --  EFI_PEI_SERVICES pointer
    @param This --  PEI_USB_HOST_CONTROLLER_PPI pointer
    @param PortNumber --  Port number whose feature is to be set
...
{    
    ...
    switch (Attributes) {
        case EFI_USB_HC_RESET_GLOBAL:   
            ...
            // Check HC is running, if so STOP before proceeding
...
/**
    ...
    @param fpBuf   - 32bit absolute buffer pointer
    @param wSize   - Amount of data to be transferred
...
)
{
    ...
    //
    // Make sure the amount of data to be transfered is 16K or less
    //
    ...
    for (;;)
    {
        *fpBufferPtr = (UINT32)(UINTN)fpBuffer;
        ...
        //
        // Check whether all the bytes can be accommodated in the current buffer
        //
...
)
{
    ...
    *TransferResult = EFI_USB_NOERROR;
    ...
    //
    // Initialize the queue head with null pointers
    //
    ...
    if (DeviceSpeed != USB_HIGH_SPEED_DEVICE && TransactionTranslator != 0)
    {
        ...
        //
        dTmp1 = (TransactionTranslator << 16) | (BIT10+BIT11+BIT12);    // Split complete transaction
    ...
    //   which was before initialized to contain a DeviceRequest struct.
    ...
    // for IN control transfer, status should be OUT
    //
...
)
{
    ...
    *TransferResult = EFI_USB_NOERROR;
    ...
    if (DeviceSpeed != USB_HIGH_SPEED_DEVICE && TransactionTranslator != 0)
    {
        // Note: low speed bulk end points are not supported
        ...
        dTmp |= dTmp1;
            ...
            // Set the hub address and port number
            //
        dTmp1   = (TransactionTranslator << 16) | BIT14;    // High speed hub port number & device number
        fpQHBulk->dEndPntCap |= dTmp1;   // Split complete transaction
    }
...
/**
    ...
    @param bOffset     - Offset of dBaseAddr
