...
)
{
    switch(VerificationStatus) {
        case 1:
            return FW_UPDATE_STATUS_INVALID_FORMAT;
        case 2:
            return FW_UPDATE_STATUS_AUTHENTICATION_ERROR;
        case 3:
            return FW_UPDATE_STATUS_AUTHENTICATION_ERROR;
        case 4:
            return FW_UPDATE_STATUS_INCORRECT_VERSION;
        default:
            break;
    }
    return FW_UPDATE_STATUS_UNKNOWN_ERROR;
}
...
)
{
    ...
    Status = pRS->GetVariable(FW_VERSION_VARIABLE, &gAmiGlobalVariableGuid, NULL, &Size, &FwStatus);
    if(!EFI_ERROR(Status)) {
        EsrtTable->Entries[0].LastAttemptVersion = FwStatus.Version;
        EsrtTable->Entries[0].LastAttemptStatus = GetErrorCode(FwStatus.Status);
    } else {
...
/* set version we're upgrading to */
    FwStatus.Version = GetVersionFromFid(RecoveryBuffer);
