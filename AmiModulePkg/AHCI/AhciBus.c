...
)
{
	...
	//DJH002021++>>
    #if defined ( Baidu_SUPPORT ) && ( Baidu_SUPPORT == 1 )
		if (!EFI_ERROR(Status)) { a[CurrentPort] = 1;}    
    #endif
	//DJH002021++<<
...
NextDevice:
    ...
    //DJH002022++>>
  #if defined ( Baidu_SUPPORT ) && ( Baidu_SUPPORT == 1 )
    PcdSet8 (DiskController,i);
	TRACE_AHCI ((-1, " Set DiskController: %0d \n",PcdGet8 (DiskController) ));
  #endif
    //DJH002022++<<
...
)
{
...
#if !OEM_SEL_LOG_SUPPORT  //TH0179++
    //TH0053>>>
  	EFI_GUID gEfiGenericElogProtocolGuid = EFI_SM_ELOG_PROTOCOL_GUID;
  	EFI_SM_ELOG_PROTOCOL        *mGenericElogProtocol = NULL;
  	UINT64                       RecordId;
  	UINT8 Record = EFI_EVENT_LOG_TYPE_OEM_IDE_DEVICE_FAIL;   //Example for Error Type. You could modify for need.
  	//TH0053<<<
#endif	//TH0179++
    ...
    if (EFI_ERROR(Status)) { 
        //TH0053 ERROR_CODE(DXE_IDE_DEVICE_FAILURE, EFI_ERROR_MAJOR);
//TH0179>>>   	
    	  //
		    //Log:Unrecoverable hard dik/ATAPI/IDE device failure into SEL(AHCI mode).  
		    //
#if OEM_SEL_LOG_SUPPORT    	
...
//TH0179>>>    	
    	//
    	//Log:Unrecoverable hard disk controller failure into SEL(AHCI mode). 
    	//
#if OEM_SEL_LOG_SUPPORT 
...
#if defined(ACOUSTIC_MANAGEMENT_DRIVER_SUPPORT) && (ACOUSTIC_MANAGEMENT_DRIVER_SUPPORT != 0)
         if(gHddAcousticInitProtocol != NULL) {
              gHddAcousticInitProtocol->InitHddAcoustic(SataDevInterface, TRUE);
         }
#endif
