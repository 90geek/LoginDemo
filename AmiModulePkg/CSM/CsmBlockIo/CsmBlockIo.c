    ...
    )
{
    ...
    //AptioV server override start: Fiber Channel controllers support in CsmBlockIo 
	    if ( Pci.Header.ClassCode[1] ==  PCI_CL_SER_BUS_SCL_FIBCHAN &&
	        Pci.Header.ClassCode[2] == PCI_CL_SER_BUS ) 
	    {
	        Status = EFI_SUCCESS;
	    }
    //AptioV server override end: Fiber Channel controllers support in CsmBlockIo 
    return Status;
}
