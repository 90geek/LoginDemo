...
)
{
    ...
    if (EFI_ERROR(Status) || ((RegionBase + Length) > (RegionLimit + 1))) return Status;
...
{
    static UINTN    SavedBiosLimit = 0, SavedFlashLimit = 0;
    UINTN           MaxFlashLimit = SavedFlashLimit, BiosRegionLimit = SavedBiosLimit;
    ...
    if (!SavedBiosLimit) {
        ...
        if (EFI_ERROR(GetFlashRegion(FlashRegionBios, NULL, &BiosRegionLimit))) {
            BiosRegionLimit = (mExFlashPart.FlashCapacity - 1);
            MaxFlashLimit = mExFlashPart.FlashCapacity;
        } else GetMaxRegionLimit(&MaxFlashLimit);
        SavedBiosLimit = BiosRegionLimit;
        SavedFlashLimit = MaxFlashLimit;
    }
    if ( gbDeviceVirtual ) {
    ...
    pAddress += (BiosRegionLimit + 1);
    // check if address inside the flash region.
    if ((UINT32)pAddress > (UINT32)MaxFlashLimit) return EFI_INVALID_PARAMETER;
    ...
    for ( wSector = 0; wSector < wNumSectors ; wSector++ ) {
        ...
        for ( wEraseRetry = 0; wEraseRetry < FLASH_RETRIES; wEraseRetry++ ) {
            ...
            *(volatile UINT16*)( gSPIBASE + SPIRegs->SwSeqCtrl ) = wSpiCmd;
            ...
            // and compare.
            if (IsEqualOrClean((volatile UINT8*)dSectorAddr, NULL, 1, TRUE)) break;
        }
...
)
{
    ...
    for ( bFlashRetry = 0; bFlashRetry < FLASH_RETRIES; bFlashRetry++ ) {
        ...
        // Wait for spi hardware/software sequence cycle completed.
        WaitForSpiCycleDone (TRUE, TRUE);
        if (EFI_ERROR(CommonConvertSpiAddress (pByteAddress, &dPhyAddress))) return;
        *(volatile UINT32*)( gSPIBASE + SPIRegs->FlashAddr ) = dPhyAddress;
#if SPI_USE_HARDWARE_SEQUENCE != 0
...
{
    UINT8           blSpiCycleRead = FALSE;
    
#if defined NO_MMIO_FLASH_ACCESS_DURING_UPDATE && NO_MMIO_FLASH_ACCESS_DURING_UPDATE == 1
    blSpiCycleRead = TRUE;
#else // NO_MMIO_FLASH_ACCESS_DURING_UPDATE == 0
    // Check the Top Swap enabled only if device is not converted to virtual for below 
    // reasons. 1) So Far TopSwap bit is set only if BIOS is in Recovery not in runtime.
    // 2) The IsTopSwapOn() could not be converted to virtual then could cause General 
    // Protection Fault in runtime.
    if (!gbDeviceVirtual && IsTopSwapOn()) blSpiCycleRead = TRUE;
#endif // #if defined NO_MMIO_FLASH_ACCESS_DURING_UPDATE && NO_MMIO_FLASH_ACCESS_DURING_UPDATE == 1
    if (!IsEqualOrClean (pBlockAddress, NULL, FlashBlockSize, blSpiCycleRead)) {
...
)
{
    ...
    if (!gbDeviceVirtual && IsTopSwapOn()) blSpiCycleRead = TRUE;
#endif // #if defined NO_MMIO_FLASH_ACCESS_DURING_UPDATE && NO_MMIO_FLASH_ACCESS_DURING_UPDATE == 1
    if (!IsEqualOrClean (pByteAddress, Byte, Length, blSpiCycleRead)) {
        if ( pStatus ) *pStatus = EFI_NOT_READY;
        if ( pError ) *pError = TRUE;
        return FALSE;
    }
