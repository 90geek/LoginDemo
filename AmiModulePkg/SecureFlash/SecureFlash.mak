...
.PHONY : SecureModule
.PHONY : CLEAN_FWCAPSULE_FILE AMI_MAKE_SIGN_ROM_BATCH_FILE MOD_FWCAPSULE_HDR_FFS $(FWCAPSULE_FILE_NAME)

ifeq ($(UNSIGNED_BIOS_ROM),)
UNSIGNED_BIOS_ROM = $(BUILD_DIR)$(PATH_SLASH)$(PLATFORM_NAME)$(PATH_SLASH)$(TARGET)_$(TOOL_CHAIN_TAG)$(PATH_SLASH)FV$(PATH_SLASH)AMIROM.fd
endif
...
ifneq ($(wildcard $(FWCAPSULE_FILE_NAME)),$(FWCAPSULE_FILE_NAME))
	$(RM) $(FWCAPSULE_FILE_NAME)
endif	
ifneq ($(wildcard make_sign_capsule*.*),make_sign_capsule*.*)
...
$(EOL)           Placeholder Platform Key FFS file (FV_BB) is created with the $(FWKEY_FILE_SIZE) byte Dummy Key\
$(EOL)\
$(EOL)WARNING!!! Flash update will fail with '$(notdir $(UNSIGNED_BIOS_ROM))' BIOS\
$(EOL)           unless Dummy Key is replaced with a valid BIOS Sign Key."\
>> make_sign_capsule_error.log
endif
...
$(EOL)    $(notdir $(FWrootKey)) - Root Key Certificate key (full RSA key)"\
>> make_sign_capsule_readme.txt
endif
	@$(ECHO) "    $(notdir $(FWpriv)) - Signing Certificate key (full RSA key)\
$(EOL)    $(notdir $(FWpub)) - Signing Certificate key (public key part).\
$(EOL)\
$(EOL)=============================================================\
...
#    Signtool sign /fd sha256 /p7 $(BUILD_DIR) /p7co 1.2.840.113549.1.7.1 /p7ce DetachedSignedData /csp "Hardware Cryptography Module" /f SigCert.cer /k KeyContainerName $(BUILD_DIR)\fwcap_serialized
...
#####
#
	@if not exist $(FWCAPSULE_FILE_NAME) @echo ERROR!!!   Failed to create signed BIOS Image.
#	
...
#	@if exist $(FWCAPSULE_FILE_NAME) $(CP) $(FWCAPSULE_FILE_NAME) $(UNSIGNED_BIOS_ROM)
#endif
