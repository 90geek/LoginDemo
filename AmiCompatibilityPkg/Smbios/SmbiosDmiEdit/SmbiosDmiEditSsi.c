...
{
    // Make sure PtrArray is using correct un-converted pointer from nvram
...
GetFlashDataLocation (VOID)
{
    ...
    for (i = 0; i < FV_BB_BLOCKS; i++) {
        gFlashBlockPtr = (UINT8*)(i * FLASH_BLOCK_SIZE);
...
)
{
    ...
    FlashDeviceWriteEnable();
    FlashBlockWriteEnable(gVirtFlashBlockPtr);

    FlashEraseBlock(gVirtFlashBlockPtr);

    if (FlashProgram(gVirtFlashBlockPtr, Data, FLASH_BLOCK_SIZE)) {
        Status = EFI_SUCCESS;
    }
    else {
    ...
    FlashBlockWriteDisable(gVirtFlashBlockPtr);
    FlashDeviceWriteDisable();
