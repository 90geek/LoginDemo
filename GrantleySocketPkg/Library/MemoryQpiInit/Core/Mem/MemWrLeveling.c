          ...
          if (host->var.mem.faultyPartsFlag[ch] & (1 << strobe)) {
            ...
            if (faultyPartsStatus == FPT_NO_ERROR) {
			  ...
			  //CDL002038 ++ >>
				    ...
				    FxnDisMemSel(host, socket, ch, dimm, WARN_FPT_UNCORRECTABLE_ERROR);
				  #endif
			  //CDL002038 ++ <
              //
              ...
              )
{
  ...
  for (ch = 0; ch < MAX_CH; ch++) {
    ...
    if(ShouldChipDisableDDR4LRDIMMWriteLeveling(host, socket, ch, dimm, rank)){
      ...
      //
      UINT16              MR1_WL_CLR = 0x00; // AptioV server override - Cpp Check
      MR1_WL_CLR &= 0xff7f;
      ...
      for (rank = 0; rank < (*dimmNvList)[dimm].numRanks; rank++) {
        ...
        while (AllGood == 0) {
          ...
          if (AllGoodLoops > 3) {
			...
			//CDL002038 ++ >>
			  #if defined (Ali_1218_SUPPORT) && (Ali_1218_SUPPORT == 1) 
			    FxnDisMemSel(host, socket, ch, dimm, WARN_WR_LEVEL);
			  #endif
			//CDL002038 ++ <
            AllGood = 1;
          }
