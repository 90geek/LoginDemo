      ...
      if (host->setup.mem.enforcePOR != ENFORCE_POR_DIS) {
        for (ch = 0; ch < MAX_CH; ch++) {
          ...
          if(tmpFreq == NOT_SUP) {
			...
			//CDL002038 ++ >>
            #if defined (Ali_1218_SUPPORT) && (Ali_1218_SUPPORT == 1) 
            	FxnDisMemSel(host, socket, ch, 0, WARN_DIMM_COMPAT);
            #endif
            //CDL002038 ++ <<
...
//CDL002038 ++ >>
	...
	--*/
	{
	  ...
	  rcPrintf ((host, "[FXN-Entered]: FxnDisMemSel()...\n"));
	  ...
	  SelRecordData.SensorType = EFI_MEMORY_SENSOR_TYPE;//
	  SelRecordData.SensorNumber = 0xE2;
	  SelRecordData.EventDirType = EFI_SENSOR_TYPE_EVENT_CODE_DISCRETE;
	  
	  
	  DEBUG ((EFI_D_LOAD, "[FXN] socket:%x ch:%x dimm:%x warningCode:%x\n",socket, ch, dimm, warningCode));
	  SelRecordData.OEMEvData1 = 0x04;//memory device disabled: 0x04
	  SelRecordData.OEMEvData2 = (UINT8)((socket << 6) | (ch << 3) | dimm);
	  SelRecordData.OEMEvData3 = warningCode;
	  
	  DEBUG ((EFI_D_LOAD, "[FXN] Prepare SelRecordData ok \n"));
	  DEBUG ((EFI_D_LOAD, "[FXN] OEMEvData1:%x \n",SelRecordData.OEMEvData1));
	  DEBUG ((EFI_D_LOAD, "[FXN] OEMEvData2:%x \n",SelRecordData.OEMEvData2));
	  DEBUG ((EFI_D_LOAD, "[FXN] OEMEvData3:%x \n",SelRecordData.OEMEvData3));
	  ...
	  DEBUG ((EFI_D_LOAD, "[FXN] gEfiPeiIpmiTransportPpiGuid Status: %r \n", Status));
	  ...
	  SERVER_IPMI_DEBUG ((EFI_D_LOAD, "[FXN] IpmiTransportPpi->SendIpmiCommand Status: %r \n", Status));
	  ASSERT_EFI_ERROR (Status);
	  
	  rcPrintf ((host, "[FXN-End]: FxnDisMemSel()...\n"));
