...
{ 
  ...
  UINT8  IOVC_REG_TABLE_HSX[]= {                        //mask      value    
    ...
    IOVC_RX_CTL1_QPIAGENT_DFX_PHY_IOVC_REG,             0xFD,         (1 << 1), //AptioV server override: Intel QPI Link Re-training After a Warm Reset or L1 Exit May be Unsuccessful
    // fwdc_rx_clkdeten_ovr - Bits[1,1] = 1
    IOVC_RX_CTL1_QPIAGENT_DFX_PHY_IOVC_REG,             0xFE,         (1 << 0), //AptioV server override: Intel QPI Link Re-training After a Warm Reset or L1 Exit May be Unsuccessful
    // fwdc_rx_clkdeten - Bits[0,0] = 1    
  };
  ...
  switch (host->setup.qpi.PhyLinkPerPortSetting[Socket].Phy[LinkIndex].QpiLinkSpeed) {
    case SPEED_REC_96GT:
      ...
      if (((host->var.common.cpuType == CPU_HSX) && (host->var.common.cpuSubType == CPU_HSX_EX) && (host->var.common.stepping >= C0_REV_HSX)) ||
          ((host->var.common.cpuType == CPU_BDX) && (host->var.common.cpuSubType == CPU_BDX_EX))) {
        IovcLcpkamp.Bits.fwdc_lcampen = 1;  //For 9.6GT, always enable LC Peak Amplifier (LCbypass).   HSD-4969961.
      }
      break;
...
--*/
{
 ...
 if ((host->var.common.cpuType == CPU_HSX) && (host->var.common.stepping < C0_REV_HSX)) {        
  ...
  if ((host->var.common.cpuType == CPU_HSX && host->var.common.stepping >= C0_REV_HSX)) {
    ...
    LbcPerIouData.Data = QpiReadPciCfg (host, Socket, LinkIndex, LBC_PER_IOU_DATA_QPIAGENT_DFX_PHY_REG);
    // For LBC 59, apply 0x3A00 value to HSX-EX- running at non-9.6 GT/s and apply 0x3E00 value 
    // for HSX-EP- at all speeds and EX- running at 9.6 GT/s
    if ((host->var.common.cpuType == CPU_HSX && host->var.common.cpuSubType == CPU_HSX_EX) &&
        (host->setup.qpi.PhyLinkPerPortSetting[Socket].Phy[LinkIndex].QpiLinkSpeed != SPEED_REC_96GT)) {
      LbcPerIouData.Bits.lbc_data = 0x3A00;  //HSX W/A (s4986848,s4969272)
    } else {
      LbcPerIouData.Bits.lbc_data = 0x3E00;  //HSX W/A (s4969961)
    } // HSX-EX- running at 9.6 GT/s check
    QpiWritePciCfg (host, Socket, LinkIndex, LBC_PER_IOU_DATA_QPIAGENT_DFX_PHY_REG, LbcPerIouData.Data);
