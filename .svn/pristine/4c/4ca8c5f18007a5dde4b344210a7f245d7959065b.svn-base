...
EFI_STATUS InstallUsbLegacyInterrupt (
...
EFI_STATUS InstallUsbInterrupt (
	IN AMI_USB_ISR_PROTOCOL	*This
)
{
    EFI_STATUS            Status;
    ...
    if (IrqEnableStore&IrqEnableTemp){
        MemCpy((UINT32*)UrpTempOffset, &Params, sizeof(URP_STRUC));
        return EFI_SUCCESS;
    }
#if USB_RUNTIME_DRIVER_IN_SMM  
    if (!gSmmCtl){
...
EFI_STATUS EFIAPI UsbIrqInitEntry (
...
)
{
    ...
    Status = pBS->LocateProtocol(&gEfiLegacy8259ProtocolGuid, 0, &mLegacy8259);
    ASSERT_EFI_ERROR(Status);

    // Find the CPU Arch Protocol
    Status = pBS->LocateProtocol(&gEfiCpuArchProtocolGuid, 0, &CpuArch);
    ASSERT_EFI_ERROR(Status);

    // Find the AMI USB protocol.
    Status = pBS->LocateProtocol(&gEfiUsbProtocolGuid, NULL, &gAmiUsbController);
    ASSERT_EFI_ERROR(Status);
    ...
    Status = pBS->LocateProtocol(&gEfiLegacyBiosProtocolGuid, 0, &LegacyBios);
    if (!EFI_ERROR(Status)){
         LegacyBiosInstall = TRUE;
    } else {
    ...
    Status = CreateReadyToBootEvent(TPL_CALLBACK, UsbIsrReadyToBoot, NULL, &ReadyToBoot);
