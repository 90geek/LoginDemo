...
)
{
    ...
    if (ModeFlag) {
		...
		Controller = PcdGet8 (DiskController); //DJH002022++
		TRACE ((-1, "Set DiskController: %0d \n",Controller)); 
		//DiskError stand for Disk Error,set bit to 1 mean which Disk Error
		if( Controller == 4) {   
		    	if(ChannelorPort == 0) DiskError = DiskError | 1;
		    	if(ChannelorPort == 1) DiskError = DiskError | (1<<1);
		    	if(ChannelorPort == 2) DiskError = DiskError | (1<<2);
		    	if(ChannelorPort == 3) DiskError = DiskError | (1<<3);
		   
		    	gDiskError = DiskError;
		    	PcdSet16 (PchDiskError,DiskError);
		    	TRACE((-1, " Set sSATA DiskError: %0x \n",gDiskError));
		    } else if (Controller == 6){
		    	DiskError = gDiskError;
		    	if(ChannelorPort == 0) DiskError = DiskError | (1<<4);
		    	if(ChannelorPort == 1) DiskError = DiskError | (1<<5);
		    	if(ChannelorPort == 2) DiskError = DiskError | (1<<6);
		    	if(ChannelorPort == 3) DiskError = DiskError | (1<<7);
		    	if(ChannelorPort == 4) DiskError = DiskError | (1<<8);
		    	if(ChannelorPort == 5) DiskError = DiskError | (1<<9);
		    	
				PcdSet16 (PchDiskError,DiskError);
				TRACE((-1, " Set SATA PchDiskError: %0x \n",DiskError));
		    }
		  #endif
	//DJH002022++<<
    } else {
	...
	Str1 = HiiGetString(HiiPostHandle, STRING_TOKEN(STR_SMART_FAILURE));
        ...
        if (pST->ConIn != NULL) {
            ...
            //DJH001001++>>
            #if defined ( Baidu_SUPPORT ) && ( Baidu_SUPPORT == 1 )
            pBS->Stall(50000*POST_ERROR_DELAY_MULIPLE);
            #else  
            //DJH002004++<<
            do {
                //
