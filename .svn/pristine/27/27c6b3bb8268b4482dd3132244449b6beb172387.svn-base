...
#define MAX_TABLE_ENTRIES	5120 //4096 AptioV server override: 4K entries is insufficient for Broadwell 4S EP. 
...
VOID * AllocTableEntry(IN UINTN Size)
{
                                                                ...
                                                                )
	{
        ...
        if (gInSmm != FALSE) return NULL; // we can't allocate memory in SMM
...
//Aptio V Override update kernel fix start.
...
//Aptio V Override update kernel fix end
...
//-------------------------------	
    ...
    if(EFI_ERROR(Status)) TRACE((-1,"EFI_S3_SMM_SAVE_STATE_PROTOCOL installation Error\n"));
	...
	)
{
	...
	TRACE((-1,"InitBootScript.\n"));

    return InitSmmHandlerEx(
                   ImageHandle, SystemTable, InitBootScriptSmm, InitBootScriptDxe
               );
}
