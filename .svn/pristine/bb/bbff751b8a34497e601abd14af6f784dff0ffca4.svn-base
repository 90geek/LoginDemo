...
//  FWKey : hash
        if(!guidcmp(&KeyHndl.AlgGuid, &gEfiCertSha256Guid) ){
            ...
            // SHA256 Hash of RSA Key
                sha256_vector(1, (const UINT8**)&PublicKey->Blob, (const size_t*)&PublicKey->BlobSize, KeyBuf);
            } 
        } else 
// FWKey : x509        
        ...
        //    pubkey : Rsa2048 (Pkcs1) - xtract x509 pub key
            if(!guidcmp(&PublicKey->AlgGuid, &gEfiCertRsa2048Guid) ){
                ...
                KeyBuf = PublicKey->Blob;
                ResetCRmm();
                err = Pkcs7_x509_return_Cert_pubKey((UINT8*)KeyHndl.Blob, (UINTN)KeyHndl.BlobSize, &PKpubBuffer, &KeyLen);
                ResetCRmm();
#endif                
