  ...
  IN  CONST EFI_PEI_SERVICES        **PeiServices )
{
    ...
    do {
        Status = (*PeiServices)->LocatePpi(
                    ...
                    NULL,
                    &BlockIoPpi );
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gEfiPeiVirtualBlockIoPpiGuid Status %r\n", Status));
        //
        // For error case, print the error status and return
        //
        if ( EFI_ERROR(Status) ) {
            SERVER_IPMI_DEBUG ((EFI_D_LOAD, "BMC USB interface is not found.\n"));
        ...
        //
        if ( IsBmcUsbInterface (PeiServices, PeiBotDevice) ) {
            break;
        }
    ...
    if ( IpmiUsbInstance == NULL ) {
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Failed to allocate memory for IpmiUsbInstance! \n"));
    ...
    if ( PeiIpmiUsbPpiDesc == NULL ) {
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Failed to allocate memory for PeiIpmiUsbPpiDesc! \n"));
        return EFI_OUT_OF_RESOURCES;
    }
    ...
    IpmiUsbInstance->Context = PeiServices;
    ...
    for (Index = 0; Index < IPMI_SELF_TEST_COMMAND_RETRY_COUNT; Index++) {
        ...
        Status = IpmiUsbInstance->IpmiUsbTransport.SendIpmiCommand (
                    ...
                    NULL,
                    0,
                    IpmiUsbInstance->TempData,
                    &DataSize );
        DEBUG ((EFI_D_LOAD, "Get Self test results Index: %x Status %r\n", Index, Status));
        if ( !EFI_ERROR(Status) ) {
            break;
        }
    }
    ...
    if ( EFI_ERROR (Status) ) {
        ...
        //
        switch ( IpmiUsbInstance->TempData[0] ) {
    ...
    if ( IpmiUsbInstance->BmcStatus != EFI_BMC_HARDFAIL ) {
        ...
        Status = (*PeiServices)->InstallPpi (
                    PeiServices,
                    PeiIpmiUsbPpiDesc );
        SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Install IpmiUsbTransportPpi - Status: %r\n", Status));
        return Status;
    } else {
        //
        // SELF test has failed, so return EFI_UNSUPPORTED to unload driver from memory.
        //
        IoWrite8 (0x80, PEI_POSTCODE_BMC_SELFTEST_FAIL_IN_IPMIUSB);
        return EFI_UNSUPPORTED;
    }
}
//**********************************************************************
