  ...
  IN  CHAR16  TimeZone[] )
{
    ...
    SERVER_IPMI_TRACE ((-1,"\nEntered...ValidateTimeZone() TimeZone: %S \n", TimeZone));
    ...
    if ( (TimeZone[5] >= '0') && (TimeZone[5] <= '9') ) {
        ...
        SERVER_IPMI_TRACE ((-1," After Converting char to number - 1's position UtcOffset: %d \n", UtcOffset));
    } else {
        goto ErrorExit;
    }
    ...
    if ( (TimeZone[4] >= '0') && (TimeZone[4] <= '5') ) {
        ...
        SERVER_IPMI_TRACE ((-1," After Converting char to number - 10's position UtcOffset: %d \n", UtcOffset));
    } else {
        goto ErrorExit;
    }
    ...
    if ( (TimeZone[2] >= '0') && (TimeZone[2] <= '9') ) {
        ...
        SERVER_IPMI_TRACE ((-1," After Converting Hour to minute - 1's position UtcOffset: %d \n", UtcOffset));
    } else {
        goto ErrorExit;
    }
    ...
    if ( (TimeZone[1] >= '0') && (TimeZone[1] <= '2') ) {
        ...
        SERVER_IPMI_TRACE ((-1," After Converting Hour to minute - 10's position UtcOffset: %d \n", UtcOffset));
    } else {
        goto ErrorExit;
    }
  ...
  IN UINT16             Class )
{
    ...
    Status = gBS->LocateProtocol (
                        ...
                        NULL,
                        (VOID**)&IpmiTransport );
    SERVER_IPMI_TRACE ((-1, " In InitBmcTimeZoneDisplay : gEfiDxeIpmiTransportProtocolGuid Status: %r \n", Status));
    ...
    if(!EFI_ERROR (Status)) {
        ...
        Status = IpmiTransport->SendIpmiCommand (
                                    ...
                                    (UINT8*)&(BmcUtcOffset),
                                    &DataSize );
        SERVER_IPMI_TRACE ((-1, " In InitBmcTimeZoneDisplay : EFI_STORAGE_GET_SEL_TIME_UTC_OFFSET Status: %r\n", Status));
        if (!EFI_ERROR (Status)) {
