...
#include <Protocol\IioUds.h>  // Aptio V Server override:  if TOLM < IOH0 MMIOL Base address
...
EFI_IIO_UDS_PROTOCOL  		*mIohUds;  // Aptio V Server override:  if TOLM < IOH0 MMIOL Base address
...
// Aptio V Server override Start
      ...
      //
    	Status = pBS->LocateProtocol(
     	...
     	if (EFI_ERROR(Status))
     	{
     		TRACE((TRACE_ALWAYS, "ConvertMemoryMap Error:Unable to find IIO UDS Protocol\n"));
...
// Aptio V Server override end
	...
	for(i=0; i<gHostBrgCount; i++){
	    ...
	    //-------------------------	
			...
			Status=LaunchInitRoutine(rb, isRbBusUpdate, itRoot, rb, &j, busrd, NULL);
			if(EFI_ERROR(Status)){
				if(Status==EFI_UNSUPPORTED){
					Status=EFI_SUCCESS;
				} else {
...
//<AMI_PHDR_END>
VOID RbReadyToBoot(IN EFI_EVENT Event,	IN VOID	*Context){
    //AptioV server override start: Update MMCFG base from PCD
    EFI_STATUS                      Status;
    ACPI_HDR                        *dsdt;
    EFI_PHYSICAL_ADDRESS            a;   
    //AptioV server override end
    
/* Aptio V Server override : AMI PCI CRS not used
    UINTN                           i,j,k;
    PCI_HOST_BRG_DATA               *hb;
    PCI_ROOT_BRG_DATA               *rb;
    ACPI_HDR                        *dsdt;
...
//Need to do : Aptio v Server Override		// if TOLM < IOH0 MMIOL Base address
		  // Adjusting GCD : Add MMIO range allocated for IOH's only.By default everything above TOLM was added.
		  // Adjust the Length also.
	           PCI_TRACE((TRACE_PCI,"GCD:MAP BASE Address=0x%lX; LEN=0x%lX;\n",pMap[i].BaseAddress,pMap[i].Length));
		   PCI_TRACE((TRACE_PCI,"IioUds->PlatformData.IIO_resource[0].PciResourceMem32Base=0x%lX;\n",mIohUds->IioUdsPtr->PlatformData.IIO_resource[0].PciResourceMem32Base));
		    if ((mIohUds->IioUdsPtr->PlatformData.IIO_resource[0].PciResourceMem32Base > pMap[i].BaseAddress) && 
		            (mIohUds->IioUdsPtr->PlatformData.IIO_resource[0].PciResourceMem32Base < (pMap[i].BaseAddress +  pMap[i].Length)))
		    {
		      PCI_TRACE((TRACE_PCI,"GCD MMIO Base address is less than IOH0 Base.GCD Base address is changed to IOH0 Base\n"));
		      PCI_TRACE((TRACE_PCI,"GCD:MAP BASE Address=0x%lX; IOH0 Base=0x%lX;\n",pMap[i].BaseAddress,mIohUds->IioUdsPtr->PlatformData.IIO_resource[0].PciResourceMem32Base));
		      pMap[i].Length = pMap[i].Length - (mIohUds->IioUdsPtr->PlatformData.IIO_resource[0].PciResourceMem32Base - pMap[i].BaseAddress);
		      pMap[i].BaseAddress = mIohUds->IioUdsPtr->PlatformData.IIO_resource[0].PciResourceMem32Base;
		      PCI_TRACE((TRACE_PCI,"GCD:MAP BASE Address=0x%lX; LEN=0x%lX;\n",pMap[i].BaseAddress,pMap[i].Length));
...
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
//AptioV Server override Start: Changes to pass UEFI SCT 2.3 PCI test
...
//    PCI_TRACE((TRACE_PCI, "\nPciRb: SetAttributes('%s'; Attr=0x%lX; Capab=0x%lX; ResBs=0x%lX; ResLn=0x%lX); Status=", 
...
//--------------------------------------------------
//AptioV Server override End: Changes to pass UEFI SCT 2.3 PCI test
