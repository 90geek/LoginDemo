...
EFI_STATUS FixedBootOrderToBoot(IN void)
{
    ...
    if (!EFI_ERROR(Status))
        NewBootOrderSzie = UefiDevSize;  // number of boot orders = size (which is definitely beyond the real size)

    // max size of BootOrder = UEFI + legacy boot orders
    NewBootOrderSzie += gBootOptionCount;
    NewBootOrder = MallocZ(NewBootOrderSzie);
        ...
        if (UefiDevOrderBuffer != NULL)
        {
            for (i = 0; i < DevCount; i++)
            {
                ...
                else
                {
                    ...
                    // Get FboLegacyDevOrder
                    Status = GetEfiVariable(L"FboLegacyDevOrder", &FixedBootOrderGuid, &Attr, &Size, &DevOrderBuffer);
                    if(EFI_ERROR(Status))
                        return EFI_SUCCESS;
                    ...
                    for (j = 0; j < gBootOptionCount; j++)
                    {
                        ...
                        DevOrder = (LEGACY_DEVICE_ORDER*)((UINT8*)DevOrderBuffer + gBootData[j].LegacyEntryOffset);
                        if (pFBODualDevMap[DevIndex].DevType == DevOrder->Type)
                        {
...
#if FIXED_BOOT_INCLUDE_DISABLED_OPTION
    {
        ...
        UINT16 *TempBootOrder = MallocZ(NewBootOrderSzie);
        UINT16 TempIndex = 0 ;
...
/**
    Restore the device type of LegacyDevOrder 
...
EFI_STATUS FixedBootOrderSaveChange(IN void)
{
    ...
    LEGACY_DEVICE_ORDER *LegacyDevBuffer;
    ...
    LegacyDevBuffer = LegacyGroup = MallocZ(Size);
    ...
    if (!EFI_ERROR(Status) && Size >= EFI_FIELD_OFFSET(LEGACY_DEVICE_ORDER,Device))
    {
        ...
        Status = pRS->SetVariable(L"FboLegacyDevOrder", &FixedBootOrderGuid, BOOT_NV_ATTRIBUTES, Size, LegacyDevBuffer);
        // Priorities changed !! Sync with LegacyDevOrder
        {
            ...
            for (DevOrder = LegacyDevBuffer
                            ; (UINT8*)DevOrder < (UINT8*)LegacyDevBuffer + Size
                        ; DevOrder = (LEGACY_DEVICE_ORDER*)((UINT8*)DevOrder + DevOrder->Length + sizeof(DevOrder->Type)))
            {
                OrderData = (UINT16*) & DevOrder->Device[0];
        ...
        Status = pRS->SetVariable(L"LegacyDevOrder", &LegacyDevOrderGuid, 
                LegacyDevAttr, Size, LegacyDevBuffer);
        ...
        // DEBUG: list the saved FboLegacyDevOrder
        {
            ...
            for (j = 0; j < Size; j++)
                TRACE((FBO_TRACE_LEVEL, "%02x ", *((UINT8*)LegacyDevBuffer + j)));
            TRACE((FBO_TRACE_LEVEL, "\n"));
        }
#endif
