...
#define KCS_DATA_PORT        IpmiInstance->Interface.KCS.DataPort
#define KCS_COMMAND_PORT     IpmiInstance->Interface.KCS.CommandPort
  ...
  IN  VOID                          *Context )
{
    ...
    while (RetryCount < KCS_ABORT_RETRY_COUNT) {
        ...
        while ( TRUE ) {
            KcsStatus.RawData = IoRead8 (KCS_COMMAND_PORT);
            ...
            //TH0072A            MicroSecondDelay (3*100);	//TH0072
        } //while loop: IBF flag
        ...
        //
        KcsData = KCS_ABORT;
        IoWrite8 (KCS_COMMAND_PORT, KcsData);
        ...
        //
        KcsStatus.RawData = IoRead8 (KCS_DATA_PORT);

        //
        // Error exit Flow chart: 00h to DATA_IN; phase = error2
        //
        KcsData = 0x0;
        IoWrite8 (KCS_DATA_PORT, KcsData);
  ...
  OUT UINT8                           *DataSize )
{
    ...
    while (TRUE) {
        ...
        //
        Data[Count] = IoRead8 (KCS_DATA_PORT);
        Count++;
