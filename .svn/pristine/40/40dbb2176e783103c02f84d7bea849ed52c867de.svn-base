...
VOID InitPostScreen( VOID )
{
   ...
   UINTN       LogoType = 0;
	...
	//EIP-197833 If any SwitchToPostScreen happens during ProceedToBoot then taking backup of original gPostStatus before changing to IN_POST_SCREEN
	...
	//EIP-197833 If SwitchToPostScreen is called during TSE_POST_STATUS_PROCEED_TO_BOOT poststatus then we need to save PROCEED_TO_BOOT to set it back after drawing postlogo to avoid drawing msgbox, message, other postinterfaces
...
VOID InstallClickHandlers( VOID )
{
	...
	Status = TimerCreateTimer( &gClickTimer, CheckForClickHook, NULL, TimerPeriodic, TIMER_HALF_SECOND, EFI_TPL_NOTIFY );
	// Signal it once immediately.
    if (!EFI_ERROR(Status))
...
//<AMI_PHDR_START>
//----------------------------------------------------------------------------------------------------------------
// Procedure:	ReInstallBgrt
//
// Description:	Reinstall the BGRT status field if BGRT is uinstalled when booting legacy boot options
//
// Input:		VOID
//
// Output:		VOID
