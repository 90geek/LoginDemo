...
VOID                        *UpdateComRegistration = NULL;  //AptioV server override - LCR does not work with Terminal module update
...
//AptioV server override start- LCR does not work with Terminal module update
/**
    This call back will update the gAmiComParameters after all drivers are connected in BDS, so that it will 
    be used at legacy boot event call.
...
)
{
    ...
    // Give a Dummy call to fill gAmiComParameters so that the data is available for Legacy_OS_SerialRedirection().
    // We save this because the NVRAM variables are cleared by Terminal driver by DisconnectSerialIo() at legacy Boot in csm.
    GetSetupValuesForLegacySredir(&AmiComParameters);
    
    pBS->CloseEvent(Event);

}
//AptioV server override end- LCR does not work with Terminal module update
/**
...
)
{
    ...
    #endif
    EFI_EVENT                        Event;
    EFI_EVENT                        UpdateComParameterEvent; //AptioV server override - LCR does not work with Terminal module update
