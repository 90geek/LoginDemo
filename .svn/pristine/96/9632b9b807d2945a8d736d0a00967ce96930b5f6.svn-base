...
VOID EfiPxeBoot(VOID)
{
    ...
    //TH0145A>>>
#if defined(NETBOOT_ON_READYTOBOOT) && (NETBOOT_ON_READYTOBOOT == 1)
    UINT16 Head, Tail, NewIndex = 0;
#endif //defined(NETBOOT_ON_READYTOBOOT) && (NETBOOT_ON_READYTOBOOT == 1)
//TH0145A<<<
...
//TH0145A>>>
    ...
    for (Index = 0; Index < BootOrderSize / sizeof(UINT16); Index++) {
        ...
        if (!EFI_ERROR(Status)) {
        	Head = pBootOrder[Index];
            for (Tail = Index; Tail > NewIndex; Tail--) {
            	pBootOrder[Tail] = pBootOrder[Tail-1];
            }
        	pBootOrder[NewIndex] = Head;
            NewIndex++;
        }
    }
    if (NewIndex) {
...
#endif //defined(NETBOOT_ON_READYTOBOOT) && (NETBOOT_ON_READYTOBOOT == 1)	//TH0145A
    pBS->FreePool(pBootOption);
...
//TH0145         do {  
        while(BbsTable[Index].BootPriority > x) {		//TH0145
            ...
            for ( i = 0; i < BbsCount; i++ ) {
                ...
                if (BbsTable[i].Class == PCI_CL_NETWORK) continue;	//TH0145
...
//TH0145         } while(BbsTable[Index].BootPriority > x);
       }	//TH0145
    }

#if !defined(NETBOOT_ON_READYTOBOOT) || (NETBOOT_ON_READYTOBOOT == 0)	//TH0145
...
#endif // !defined(NETBOOT_ON_READYTOBOOT) || (NETBOOT_ON_READYTOBOOT == 0)	//TH0145
...
//TH0145>>>
#if defined(NETBOOT_ON_READYTOBOOT) && (NETBOOT_ON_READYTOBOOT == 1)
EFI_EVENT NetBootEvent = NULL; //TH0145A
...
//TH0145A	NetBootHotKey();
    if (NetBootEvent != NULL) NetBootHotKey(); //TH0145A

}

//TH0145A   EFI_EVENT NetBootEvent = NULL;

EFI_STATUS NetBootHotKeyEx(VOID)
{
    ...
    //
    if (NetBootEvent != NULL) return Status;	//Already hooked
