  ...
  OUT GET_BOOT_FLAG_RESPONSE        *BootFlagsResponse )
{
    ...
    SERVER_IPMI_DEBUG ((-1, " IpmiGetBootFlags: Entered......\n"));

    //
    // Wait until Set In Progress field is cleared
    //
    Status = IpmiWaitSetInProgressClear ();
    SERVER_IPMI_DEBUG ((-1, " IpmiWaitSetInProgressClear: Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    ...
    Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                 ...
                 &ResponseSize );
    SERVER_IPMI_DEBUG ((-1, " IpmiGetBootFlags: EFI_CHASSIS_GET_SYSTEM_BOOT_OPTIONS Status: %r \n", Status));
    SERVER_IPMI_DEBUG ((-1, " IpmiGetBootFlags: Exiting........ Status: %r \n", Status));
   ...
   IN  BOOT_OPTIONS_BOOT_FLAGS    Param5 )
{
    ...
    ResponseSize = 1;
    RequestSize = sizeof (SET_BOOT_FLAG_REQUEST);
    Status =gIpmiInstance->IpmiTransport.SendIpmiCommand (
                                    &gIpmiInstance->IpmiTransport,
                                    EFI_SM_NETFN_CHASSIS,
                                    0,
  ...
  IN  VOID              *Context )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "ResetArchProtocolNotification entered....\n"));

    Status = gBS->LocateProtocol (
                &gEfiResetArchProtocolGuid,
                NULL,
                (VOID **)&ResetArchProtocol);
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gBS->LocateProtocol gEfiResetArchProtocolGuid protocol  status %r\n", Status));
    if (EFI_ERROR (Status)) {
        return;
    }
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "Reset ArchProtocolNotification Exiting....\n"));
