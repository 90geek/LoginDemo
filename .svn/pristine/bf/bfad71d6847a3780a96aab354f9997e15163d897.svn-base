...
// Aptio V Server Override End : Changes done to log the PCIE Errors in BERT Table	  
  ...
  for(Function = 0; Function <= PCI_FUNCTION_NUMBER_PCH_PCIE_ROOT_PORT_8; Function++) {
    if (Function != ONBOARD_VIDEO_ROOT_PORT_FUN_NUM) { // AptioV Server Override: Disable reporting of Last Boot WHEA Errors for PCH Root Port (D28:F3) to which on board video device is connected. 
      //
      // Check if device is present
      //
    ...
    } // AptioV Server Override: Disable reporting of Last Boot WHEA Errors for PCH Root Port (D28:F3) to which on board video device is connected. 
  }
}
...
// AptioV Server Override Start: Added to skip LastBoot Error Handler for cold boot.
  ...
  // AptioV server override : Start - Cpp Check
  if ( ( systemMemoryMap != NULL ) && ( systemMemoryMap->IsColdBoot != TRUE ) ) {
  // AptioV server override: End - Cpp Check
