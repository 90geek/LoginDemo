...
//---------------------------------
	...
	if(Get){
		...
		Status = pRS->SetVariable(vname, &ssg, 
				//EIP193450-	(EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS), 
				(EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS), //EIP193450+<<
				vs, &Dev->NvData);
		if(vname!=NULL) pBS->FreePool(vname);
	}
	return (s2|Status);
...
//---------------
    ...
    if(Dev->Owner->SpioSdlInfo->StdSioDevice){
        ...
        //in case of non Standard SIO  just return SUCCESS
        return EFI_SUCCESS;
}
