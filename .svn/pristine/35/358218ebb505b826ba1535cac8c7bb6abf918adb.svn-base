...
)
{
    ...
    //USB_DEBUG(DEBUG_LEVEL_3, " wr(%x):%x %x", DevAddr, Result, Urp->ApiData.EfiMassRead.SenseData);
    if (Result) {
...
USBAPI_LightenKeyboardLEDs(URP_STRUC *fpURP)
{
	...
	if (fpURP->ApiData.EfiKbLedsData.DevInfoPtr) {
		UsbKbdSetLed((DEV_INFO*)fpURP->ApiData.EfiKbLedsData.DevInfoPtr, 
...
/**
    This routine copies data from source to destination.
