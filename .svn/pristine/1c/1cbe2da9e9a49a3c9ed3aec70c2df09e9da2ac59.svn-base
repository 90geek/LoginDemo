...
)
{
    ...
    UINT32      RtRomSize;
    EFI_STATUS  Status;
     //UINT32      CurrentInt10 = 0; //AptioV Server Override
    UINT32      EbdaOffset;
    ...
    if (!IsVga) {
        if (gSetTxtMode == 2) {
           ...
           //J002 CurrentInt10 = *(UINT32*)(UINTN)0x40;
           //J002 *(UINT32*)(UINTN)0x40 = 0xf000f065;    // legacy int10
           //AptioV server override end
        } else {
            if (gSetTxtMode != 1) {
                // gSetTxtMode is either 0 or ff
                DisconnectSerialIO();
...
//TH0053>>>
    ...
    UINT64                     RecordId;
    UINT8 Record = EFI_EVENT_LOG_TYPE_LEGACY_OPROM_NO_SPACE;   //Example for Error Type. You could modify for need.
//TH0053<<<
//TH0033>>>
    UINT8        i;
    ...
    if (is30ROM) {
        ...
        if (EFI_ERROR(Status)) {
            PostProcessOpRom();
            //TH0053 DEVICE_ERROR_CODE(DXE_LEGACY_OPROM_NO_SPACE, EFI_ERROR_MAJOR, PciHandle);
            //TH0053>>>    	    	
            Status = pBS->LocateProtocol (&gEfiGenericElogProtocolGuid, 
...
TRACE((TRACE_ALWAYS, "PCI OPROM(handle %x, %x/%x/%x): addr %x, size %x\n",
    ...
    {
    	/* TH0131
	    //AptioV server Override Start: Geeric Fix - EIP#199452 Changes
	    ...
	    //AptioV server Override End: Geeric Fix - EIP#199452 Changes
      TH0131 */  
        LegacyBoot (
            &CoreBiosInfo->iBios,
...
)
{
    ...
    if (gCsmPolicy != NULL) {
	//AptioV Server Override start
        if (!EFI_ERROR(gCsmPolicy->CheckUefiOpromPolicy(gCsmPolicy, PCI_CL_DISPLAY))) {
            gSetTxtMode = 2;    // prevent video mode switching when UEFI video is selected
        }
	//AptioV Server Override end
