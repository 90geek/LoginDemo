  ...
  OUT UINT8                *ResponseDataSize )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "SmmEfiIpmiSendCommand entered  IpmiInstance: %x \n", gIpmiInstance));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gSmmCommunication->Communicate  Status: %r \n", Status));
    ASSERT_EFI_ERROR (Status);

    SmmCommunicateHeader = (EFI_SMM_COMMUNICATE_HEADER *) gCommunicateBuffer;
    SmmIpmiBmcData = (SMM_IPMI_BMC_SEND_COMMAND_COMMUNICATE_DATA *)SmmCommunicateHeader->Data;

    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "SmmIpmiBmcData->ReturnStatus : %r \n", SmmIpmiBmcData->ReturnStatus));
  ...
  OUT EFI_SM_COM_ADDRESS    *ComAddress )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "SmmIpmiGetBmcStatusData->ReturnStatus : %r \n", SmmIpmiGetBmcStatusData->ReturnStatus));
  ...
  IN  VOID              *Context )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "SmmIpmiBmcTransportProtocolReady entered IpmiInstance: %x \n", gIpmiInstance));
    Status = gBS->LocateProtocol (&gEfiSmmIpmiTransportProtocolGuid, NULL, (VOID **)&DummySmmmIpmiTransport);
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gBS->LocateProtocol gEfiSmmIpmiTransportProtocolGuid protocol  status %r\n", Status));
    if (EFI_ERROR (Status)) {
        return;
    }

    Status = gBS->LocateProtocol (&gEfiSmmCommunicationProtocolGuid, NULL, (VOID **) &gSmmCommunication);
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "gBS->LocateProtocol gEfiSmmCommunicationProtocolGuid protocol  status %r\n", Status));
    if (EFI_ERROR (Status)) {
        return;
    }
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "SmmIpmiBmcTransportProtocolReady Exiting....  IpmiInstance: %x \n", gIpmiInstance));
  ...
  VOID )
{
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "InitializeIpmiPhysicalLayerHook entered  gIpmiInstance: %x \n", gIpmiInstance));
    ...
    SERVER_IPMI_DEBUG ((EFI_D_LOAD, "InitializeIpmiPhysicalLayerHook exiting.......\n"));
    return EFI_SUCCESS;
