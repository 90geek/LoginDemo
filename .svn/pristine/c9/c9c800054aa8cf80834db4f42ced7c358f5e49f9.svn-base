...
)
{
    ...
    for (Lun = 1; Lun <= MaxLun; Lun++) {
        ...
        if ((NewDevInfo->Flag & DEV_INFO_MASS_DEV_REGD)) {
			...
			NewDevInfo->bIntEndpoint = DevInfo->bIntEndpoint;
			NewDevInfo->wIntMaxPkt = DevInfo->wIntMaxPkt;
            ...
            MemSet(NewDevInfo->DevNameString, 64, 0);
            //
...
{
    UINT8   i;
    UINT8*  Cleaner = (UINT8*)&gUsbData->stMassXactStruc;
...
)
{
    ...
    if ((DevInfo->bProtocol == PROTOCOL_CBI) ||
        ...
        return USBMassIssueCBITransaction(DevInfo);
    }

    if (DevInfo->bProtocol == PROTOCOL_BOT) {
        return USBMassIssueBOTTransaction(DevInfo);
    }
...
//  UINT8           CommandRetried = FALSE;
    ...
    //
    gUsbData->stMassXactStruc.fpCmdBuffer = CmdBuffer;
    gUsbData->stMassXactStruc.bCmdSize = (UINT8)MassCmdPassThru->wCmdLength;
    gUsbData->stMassXactStruc.bXferDir = MassCmdPassThru->bXferDir;
    gUsbData->stMassXactStruc.fpBuffer = (UINT8*)(UINTN)MassCmdPassThru->fpDataBuffer;
    gUsbData->stMassXactStruc.dLength = (UINT32)MassCmdPassThru->wDataLength;

    Data16 = (UINT16)USBMassIssueMassTransaction(DevInfo);
...
)
{
    ...
    // Fill the common bulk transaction structure
    //
    gUsbData->stMassXactStruc.fpCmdBuffer = (UINT8*)CmdBuffer;

    gUsbData->stMassXactStruc.bCmdSize = sizeof(COMN_RWV_16_CMD);
    gUsbData->stMassXactStruc.bXferDir = BIT7;     // IN
    gUsbData->stMassXactStruc.fpBuffer = gUsbData->fpUSBTempBuffer;
    gUsbData->stMassXactStruc.dLength = 0xC;
    
    USB_DEBUG (DEBUG_LEVEL_3, "rcc..");
    Data = USBMassIssueMassTransaction(DevInfo);
...
)
{
    ...
    if (DevInfo->bSubClass == SUB_CLASS_SCSI) {
        ...
        gUsbData->stMassXactStruc.bCmdSize = sizeof (COMN_READ_CAPACITY_10_CMD);
    }
										//<(EIP51158+)
    gUsbData->stMassXactStruc.bXferDir = BIT7;     // IN
    gUsbData->stMassXactStruc.fpBuffer = gUsbData->fpUSBTempBuffer;
...
)
{
    ...
    //
    gUsbData->stMassXactStruc.fpCmdBuffer = (UINT8*)CmdBuffer;
    gUsbData->stMassXactStruc.bCmdSize = sizeof (COMMON_GET_CONFIGURATION);
    gUsbData->stMassXactStruc.bXferDir = BIT7;     // IN
    gUsbData->stMassXactStruc.fpBuffer = DataBuffer;
...
)
{
    ...
    do {
        ...
        if (DevInfo->bSubClass == SUB_CLASS_SCSI) {
            ...
            gUsbData->stMassXactStruc.bCmdSize = sizeof (COMN_RWV_10_CMD);
        }
										//<(EIP51158+)
        gUsbData->stMassXactStruc.bXferDir = BIT7;     // IN
        gUsbData->stMassXactStruc.fpBuffer = Buffer;
        gUsbData->stMassXactStruc.dLength = DevInfo->wBlockSize;  //(EIP59738)
		...
		MemSet((UINT8*)CmdBuffer, sizeof(COMN_RWV_10_CMD), 0);
    } while (Counter--);
...
)
{
	...
	//
        ...
        if (DevInfo->wBlockSize != 0xFFFF) {
            ...
            //
            gUsbData->stMassXactStruc.fpCmdBuffer = (UINT8*)CmdBuffer;
            gUsbData->stMassXactStruc.bCmdSize = CmdSize;
            gUsbData->stMassXactStruc.bXferDir = Dir;
            gUsbData->stMassXactStruc.fpBuffer = (UINT8*)(UINTN)MassDataStruc->BufferPtr;
            gUsbData->stMassXactStruc.wPreSkip = MassDataStruc->PreSkipSize;
            gUsbData->stMassXactStruc.wPostSkip = MassDataStruc->PostSkipSize;
            gUsbData->stMassXactStruc.dLength = BytesToRw;
		...
		MemSet((UINT8*)CmdBuffer, sizeof(COMN_RWV_16_CMD), 0);
    }   // Fof loop
...
)
{
    ...
    if (gUsbData->dUSBStateFlag & USB_FLAG_RUNNING_UNDER_EFI) {
        if (DevInfo->MassDev) {
            (*(EFI_BLOCK_IO_PROTOCOL*)DevInfo->MassDev).Media->BlockSize = DevInfo->wBlockSize;
            if (DevInfo->bLastStatus & USB_MASS_MEDIA_PRESENT) {          
                (*(EFI_BLOCK_IO_PROTOCOL*)DevInfo->MassDev).Media->LastBlock = DevInfo->MaxLba;
            } else {
                (*(EFI_BLOCK_IO_PROTOCOL*)DevInfo->MassDev).Media->LastBlock = 0; 
            }
        }
    }
...
)
{
    ...
    Data = USBMassSendBOTCommand(DevInfo);   // Send the command control transfer
    ...
    if (!gUsbData->stMassXactStruc.dLength) {  // No data
        ...
        //
        Data = USBMassGetBOTStatus(DevInfo);
        return Data;
    }
    ...
    //
    Data = USBMassProcessBulkData(DevInfo);  // Actual data size
    ...
    //
    USBMassClearBulkEndpointStall(DevInfo, gUsbData->stMassXactStruc.bXferDir);
...
)
{
    ...
    BotCmdBlk = (BOT_CMD_BLK*)gUsbData->stMassXactStruc.fpCmdBuffer;

    CmdSize = gUsbData->stMassXactStruc.bCmdSize;
...
//        return 0;
    ...
    // Move backwards
    //
    Src = gUsbData->stMassXactStruc.fpCmdBuffer + CmdSize - 1;
    ...
    BotCmdBlk->dCbwDataLength     = gUsbData->stMassXactStruc.dLength;
    BotCmdBlk->bmCbwFlags         = gUsbData->stMassXactStruc.bXferDir;
...
)
{
    ...
    CmdBuffer = gUsbData->stMassXactStruc.fpCmdBuffer;
