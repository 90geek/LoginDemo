...
extern EFI_STATUS 
ClearSerialPortsEnabledVar_Sio(
    UINT8 Port );
    ...
    if(!EFI_ERROR(Status)) {
        ...
        do {
            ...
            if((AcpiPrevDPNodePtr->Header.Type == ACPI_DEVICE_PATH)) {
                TerminalUID = AcpiPrevDPNodePtr->UID;
                Status = ClearSerialPortsEnabledVar_Sio(TerminalUID);
...
UninstallProtocol:
	  //TH0168 Clearscreen because of an unexpected EscapeSequence printed.
    TerminalDev->SimpleTextOutput.ClearScreen(&TerminalDev->SimpleTextOutput);  //TH0168
...
)
{
    ...
    *IsPciSerialInterface = FALSE; 
    ...
    if ((AcpiPrevDPNodePtr->Header.Type == ACPI_DEVICE_PATH) &&
        ...
        //TH0108>>>
#if defined SIO_VIRTUAL_PORT_SUPPORT && SIO_VIRTUAL_PORT_SUPPORT == 1 && Thor_SUPPORT == 1

        if (AcpiPrevDPNodePtr->UID != 0){
            *ComPort = 0xff;
        } else {
