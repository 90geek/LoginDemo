...
//TH0047 >>>>   Merge USB Key into USB HDD group
/*
...
//GCH001001 + START
//----------------------------------------------------------------------------
// Procedure:   SATAPortStrSearch
//
// Description: This function scans the contents of the Null-terminated Unicode string 
//              specified by String and returns the first occurrence of SearchString.
//              If SearchString is not found in String, then NULL is returned.  If
//              the length of SearchString is zero, then String is returned.
//
// Input:       String          A pointer to a Null-terminated Unicode string
// Input:       SearchString    A pointer to a Null-terminated Unicode string to search for
//
// Output:      NULL            If the SearchString does not appear in String.
// Output:      others          If there is a match.
//----------------------------------------------------------------------------
char *SATAPortStrSearch (
  IN      CONST char              *String,
  IN      CONST char              *SearchString
  )
{
...
int SubStr( CHAR16 *string1, CHAR16 *string2 ) {
...
VOID ChangeBootOptionPriorities()
{
	DLINK       *Link=NULL;    
	UINT32      PreviousPriority=0;
	BOOT_OPTION *Option=NULL;
	BOOT_OPTION *TempOption=NULL;
	DLINK       *TempLink1=NULL;
	DLINK       *TempLink2=NULL;
	UINT32      TempPriority;
	CHAR8       *NameStr;
	UINT8       SetS00=0;
	if(BootOptionList==NULL||(BootOptionList)->pHead==NULL)
		return;
	for(  Link=((BootOptionList)->pHead);Link!=((void *) 0) && (Option=((BOOT_OPTION*)((UINT8*)(Link)-((UINTN)(&(((BOOT_OPTION *) 0)->Link))))),((BOOLEAN)(1==1)))\
	 ;)	
	{
	    NameStr = (CHAR8*)(UINTN)(( Option->BbsEntry->DescStringSegment << 4 )+ Option->BbsEntry->DescStringOffset);
	    if(NameStr==NULL)
	    {
	    	Link=Link->pNext;
	    	continue;
	    }
	    if (!SetS00&&(SATAPortStrSearch(NameStr,"S:00") != NULL)) 
	    {  
	    	TRACE((-1,"found S:00\n"));
	    	TempLink1=Link->pPrev;
	    	if(TempLink1==NULL)
	    		return;
	    	TempOption=((BOOT_OPTION*)((UINT8*)(TempLink1)-((UINTN)(&(((BOOT_OPTION *) 0)->Link)))));
	    	if((TempLink1 ==NULL)||(!SubStr(TempOption->Description,L"Hard Drive "))||TempOption->GroupHeader)
	    	{
	    		SetS00=1;      //no matter set fail or success
	    		Link=((BootOptionList)->pHead);
	    		continue;
	    	}
	    	TempLink2=TempLink1->pPrev;
	    	if(TempLink2==NULL)
	    		return;
	    	while(1)
	    	{
	    		TempPriority=TempOption->Priority;
	    		TempOption->Priority=Option->Priority;
	    		Option->Priority=TempPriority;
	    		
	    		TempLink2->pNext=Link;
	    		Link->pPrev=TempLink2;
	    		TempLink1->pNext=Link->pNext;
	    		TempLink1->pPrev=Link;
	    		Link->pNext=TempLink1;	    		
	    		TempOption=((BOOT_OPTION*)((UINT8*)(TempLink2)-((UINTN)(&(((BOOT_OPTION *) 0)->Link)))));
	    		if(TempOption==NULL)
	    			break;
	    		if((!SubStr(TempOption->Description,L"Hard Drive "))||TempOption->GroupHeader)
	    			break;
	    		TempLink1=TempLink2;
	    		TempLink2=TempLink2->pPrev;
	    		if(TempLink2==NULL)
	    			break;
	        }
	    	//return;
	    	SetS00=1;      //no matter set fail or success
	    	Link=((BootOptionList)->pHead);
	    	Option=((BOOT_OPTION*)((UINT8*)(Link)-((UINTN)(&(((BOOT_OPTION *) 0)->Link)))));
	    	NameStr = (CHAR8*)(UINTN)(( Option->BbsEntry->DescStringSegment << 4 )+ Option->BbsEntry->DescStringOffset);
	    }
	    //if cannot find S:00, still set M2 boot order
	    if(!SetS00&&((Link->pNext==NULL)||((BOOT_OPTION*)((UINT8*)(Link->pNext)-((UINTN)(&(((BOOT_OPTION *) 0)->Link)))))->GroupHeader))
	    {
		    SetS00=1;  
		    Link=((BootOptionList)->pHead);
		    Option=((BOOT_OPTION*)((UINT8*)(Link)-((UINTN)(&(((BOOT_OPTION *) 0)->Link)))));
	    ...
	    if(SetS00&&(SATAPortStrSearch(NameStr,"sSATA") != NULL))
	    {   
		TempLink1=Link->pPrev;
		TempOption=((BOOT_OPTION*)((UINT8*)(TempLink1)-((UINTN)(&(((BOOT_OPTION *) 0)->Link)))));
		if((TempLink1 ==NULL)||(!SubStr(TempOption->Description,L"Hard Drive "))||TempOption->GroupHeader)
		...
		while(1)
		{
			...
			if(TempOption==NULL)
				break;
			TRACE((-1,"TempOption->Description:%S.\n",TempOption->Description));
			if((!SubStr(TempOption->Description,L"Hard Drive "))||TempOption->GroupHeader)
			    break;
			TempLink1=TempLink2;
			TempLink2=TempLink2->pPrev;
...
VOID SetLegacyBootOptionPriority(IN UINT16 DeviceType, IN OUT UINT32 *Priority)
{
    ...
    FOR_EACH_BOOT_OPTION(BootOptionList, Link, Option)
    {
        ...
        //TH0079>>>
        // Now BootOption's Priority is always LOWEST_BOOT_OPTION_PRIORITY here.
        // If BootOption's Tag is HIGHEST_BOOT_OPTION_TAG, set the BootOption's Priority to HIGHEST_BOOT_OPTION_PRIORITY.
        if (Option->Tag == 0) {
        	Option->Priority = 0;
        	continue;
        }
...
//LCJ001045+Start
            ...
            FOR_EACH_BOOT_OPTION(BootOptionList, Link, Option)
            {
                if(Option->BbsEntry->SubClass == 0x08)
                {
                        Option->Tag = Option->Tag + 1;
                }
            }
