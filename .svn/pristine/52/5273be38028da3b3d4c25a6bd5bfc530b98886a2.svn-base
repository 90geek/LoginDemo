...
HSX_ALL_LANES_EPARAM_LINK_INFO  TempQpiMayanCityAllLanesEparamTable[] = {
   ...
   //Socket 0 **********************************************************************************************************************************************************************************************************************
         {0x0, (1 << PROBE_TYPE_NO_PROBE) | (1 << PROBE_TYPE_COHASSET) | (1 << PROBE_TYPE_VSR), 
               ...
               LINK0, 0x322808, 0x05410102, QpiSck0Lnk0TXEQ, QpiSck0Lnk0CTLE},

         {0x0, (1 << PROBE_TYPE_NO_PROBE) | (1 << PROBE_TYPE_COHASSET) | (1 << PROBE_TYPE_VSR), 
               (1 << SPEED_REC_64GT) | (1 << SPEED_REC_72GT) | (1 << SPEED_REC_80GT) | (1 << SPEED_REC_96GT), 
               LINK1, 0x322808, 0x05410102, QpiSck0Lnk1TXEQ, QpiSck0Lnk1CTLE},
         
   //Socket 1 **********************************************************************************************************************************************************************************************************************
         {0x1, (1 << PROBE_TYPE_NO_PROBE) | (1 << PROBE_TYPE_COHASSET) | (1 << PROBE_TYPE_VSR), 
               (1 << SPEED_REC_64GT) | (1 << SPEED_REC_72GT) | (1 << SPEED_REC_80GT) | (1 << SPEED_REC_96GT), 
               LINK0, 0x322808, 0x05410102, QpiSck1Lnk0TXEQ, QpiSck1Lnk0CTLE},
     
         {0x1, (1 << PROBE_TYPE_NO_PROBE) | (1 << PROBE_TYPE_COHASSET) | (1 << PROBE_TYPE_VSR), 
               (1 << SPEED_REC_64GT) | (1 << SPEED_REC_72GT) | (1 << SPEED_REC_80GT) | (1 << SPEED_REC_96GT), 
               LINK1, 0x322808, 0x05410102, QpiSck1Lnk1TXEQ, QpiSck1Lnk1CTLE}
};

UINT32 TempQpiMayanCityAllLanesEparamTableSize = sizeof (TempQpiMayanCityAllLanesEparamTable);

/**
  Get the point of QpiMayanCityAllLanesEparam table.

  @param[OUT]  **TableAddr  Return the pointer of
                            QpiMayanCityAllLanesEparam table
**/
UINT32
EFIAPI
PatchQpiMayanCityAllLanesEparamTable (
  OUT  HSX_ALL_LANES_EPARAM_LINK_INFO  **TableAddr
  )
{
  *TableAddr = TempQpiMayanCityAllLanesEparamTable;
  return TempQpiMayanCityAllLanesEparamTableSize;
}
#endif //#if defined (Ali_1218_SUPPORT)...
...
--*/
{
  ...
  if (host->setup.qpi.DebugPrintLevel & DbgLevel) {
    ...
    va_end( Marker );  // AptioV server override - Cpp Check
  }
#endif
  ...
  switch (host->nvram.common.platformType) {
    ...
    case TypeWildcatPass:
      if (host->var.common.cpuType == CPU_HSX) {
       ...
       //LLY011023+>>>
      #if defined (Ali_1218_SUPPORT) && (Ali_1218_SUPPORT == 1)
       SizeOfTable = PatchQpiMayanCityAllLanesEparamTable (&ptr); 
      #endif
     // LLY011023+<<< 
      break;
...
// For Emerald Point CRB Support
               (host->nvram.common.platformType != TypeEmeraldPoint) &&
