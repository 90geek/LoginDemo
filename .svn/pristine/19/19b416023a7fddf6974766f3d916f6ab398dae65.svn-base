...
#include <Library/BaseLib.h> //LLY011007
//TH0044>>>
#if BUILD_UEFI_BOOT_NAME_BY_DP
...
VOID CollectBbsDevices(DLIST *BootDeviceList){
    ...
    for(i = 0; i < BbsCount; i++){
		...
		if(BbsTable[i].DeviceType == BBS_FLOPPY) continue; //TH0047
        CreateBootDevice(
            BootDeviceList,
...
//<AMI_PHDR_START>
//----------------------------------------------------------------------------
// Procedure:   GetNetworkDeviceName
//
// Description: Build Network Device Boot Name according to Device Path and DIDVID
//
// Input:       EFI_DEVICE_PATH_PROTOCOL *pDp - 
//              CHAR16 *wsName -
//
// Output:      
//----------------------------------------------------------------------------
...
BOOLEAN GetNetworkDeviceName(
	IN EFI_DEVICE_PATH_PROTOCOL *pDp, 
	IN CHAR16 *wsName,
	IN EFI_HANDLE Handle)//LLY011007+
{
	MAC_ADDRESS_DEVICE_PATH       *pMACDp = NULL;
	UINT8                         DPDepth = 0;
	UINT8                         RDev;
	UINT8                         RFun;
	UINT8                         DDev;
...
//	UINT8                         DBus;
	...
	if (pDp) {
		for( ; !(isEndNode(pDp) && pDp->SubType == END_ENTIRE_SUBTYPE); pDp = NEXT_NODE(pDp)){
			if ((pDp->Type == HARDWARE_DEVICE_PATH) && (pDp->SubType == HW_PCI_DP)){				
				if (DPDepth == 0){
					RFun = *(UINT8*)(pDp + 1);
					RDev = *((UINT8*)(pDp + 1) + 1);
				} else {
			...
			if ((pDp->Type == MESSAGING_DEVICE_PATH) && (pDp->SubType == MSG_MAC_ADDR_DP)){
				pMACDp = (MAC_ADDRESS_DEVICE_PATH*)pDp;	
			}	
		}
		if ((pMACDp) && (DPDepth)) {
			switch (RDev) {
			        ...
			        case 0x3:
			        	...
			        	CHAR16                  *MellanoxP = NULL;
			        	CHAR16                  *MellanoxS = L"Mellanox";
			        	CHAR16                  *Port1 = L"P1 ";
			        	CHAR16                  *Port2 = L"P2 ";
			        	if (!GetControllerName(Handle, &ControllerName)) return NameFound;
			        	MellanoxP = StrStr(ControllerName,MellanoxS);
			        	if(MellanoxP != NULL){
			        		Swprintf(wsName, L"%s",ControllerName);
			        	}else{
			        	  if((RFun == 0x0)||(RFun == 0x2)){
			        		if(DFun == 0x00){
			        		   Swprintf(wsName, L"%s%s",Port1,ControllerName);  
					        } else if(DFun == 0x01){
					           Swprintf(wsName, L"%s%s",Port2,ControllerName);
			        		}
			        	   }
			        	}
			        	NameFound = TRUE;
				...
				//LLY011007+ <<<
				case 28:					
					// Swprintf (wsName, L"Onboard 82574 LAN%d %02X-%02X-%02X-%02X-%02X-%02X", (RFun - 4), pMACDp->MacAddr.mac[0], pMACDp->MacAddr.mac[1], pMACDp->MacAddr.mac[2], pMACDp->MacAddr.mac[3], pMACDp->MacAddr.mac[4], pMACDp->MacAddr.mac[5]);
					if(RFun == 0x04){
					   if(DFun == 0x00){
						Swprintf (wsName, L"Onboard I350 NIC1 %02X-%02X-%02X-%02X-%02X-%02X", pMACDp->MacAddr.Addr[0], pMACDp->MacAddr.Addr[1], pMACDp->MacAddr.Addr[2], pMACDp->MacAddr.Addr[3], pMACDp->MacAddr.Addr[4], pMACDp->MacAddr.Addr[5]);
				 	    } else if(DFun == 0x01){
						   Swprintf (wsName, L"Onboard I350 NIC2 %02X-%02X-%02X-%02X-%02X-%02X", pMACDp->MacAddr.Addr[0], pMACDp->MacAddr.Addr[1], pMACDp->MacAddr.Addr[2], pMACDp->MacAddr.Addr[3], pMACDp->MacAddr.Addr[4], pMACDp->MacAddr.Addr[5]);
//TH0044B>>>>					      }

				 	    }else if(DFun == 0x02){
				 	    	Swprintf (wsName, L"Onboard I350 NIC3 %02X-%02X-%02X-%02X-%02X-%02X", pMACDp->MacAddr.Addr[0], pMACDp->MacAddr.Addr[1], pMACDp->MacAddr.Addr[2], pMACDp->MacAddr.Addr[3], pMACDp->MacAddr.Addr[4], pMACDp->MacAddr.Addr[5]);
				 	    }else if(DFun == 0x03){
				 	    	Swprintf (wsName, L"Onboard I350 NIC4 %02X-%02X-%02X-%02X-%02X-%02X", pMACDp->MacAddr.Addr[0], pMACDp->MacAddr.Addr[1], pMACDp->MacAddr.Addr[2], pMACDp->MacAddr.Addr[3], pMACDp->MacAddr.Addr[4], pMACDp->MacAddr.Addr[5]);
				 	    }
//TH0044B<<<<
					}
					NameFound = TRUE;
...
#endif //#if BUILD_UEFI_BOOT_NAME_BY_DP
//TH0044<<<
...
//lly011007-        if (NameFound = GetNetworkDeviceName(DevicePath, BootName)){
        if (NameFound = GetNetworkDeviceName(DevicePath, BootName,Handle)){//lly011007+
            ControllerName = BootName;
        }
    }
    if (!NameFound){
...
//TH0044<<<
    if (!GetControllerName(Handle, &ControllerName)) return 0;
#endif //BUILD_UEFI_BOOT_NAME_BY_DP //TH0044
