  ...
  IN UINT8  UnCoreBusNumber )
{
    ...
    IIO_SVID_SID_INIT_ENTRY     OemIioSvidSidInitList[] = {
            ...
            // Add Entries here.
            //
            { FALSE, 0xFF, 0xFF, 0xFF, {0xFFFF, 0xFFFF} }
    };
	...
	if (OemIioSvidSidInitList[Index].IsCoreDevice)
		BusNumber = CoreBusNumber;
	else
		BusNumber = UnCoreBusNumber;
...
//ZA0041>>>
    ...
    EFI_STATUS Status;
    UINT8   s[32] = {0};
    UINT8   TempStr[9] = "xSATAx: ";   //"[Controller][PortNo]: "
    UINTN   Index;
    EFI_LEGACY_REGION2_PROTOCOL *LegacyRegion2Protocol;
    UINT32  Granularity;
    
    TempStr[5] = PortNumber + 0x30;
    TempStr[0] = 0x20;

    if( ControllerNo == SSATACONTROLLER ) {
        TempStr[0] = 's';

        pBS->CopyMem(s,TempStr,8);
        Index = 8;
    } else {
        pBS->CopyMem(s,&TempStr[1],7);
        Index = 7;
    }
    pBS->CopyMem(&s[Index],SerialNumber,23);
#else
//ZA0041<<<
    EFI_STATUS Status;
    UINT8   s[32] = {0};
...
//ZA0041>>>
    ...
    EFI_STATUS Status;
    UINT8   s[32] = {0};
    UINT8   TempStr[9] = "xPATAx: "; // "[Controller][PortNo]: "
    UINT8   PortNo = 0;
    EFI_LEGACY_REGION2_PROTOCOL *LegacyRegion2Protocol;
    UINT32  Granularity;
    UINT8    Index;
    ...
    PortNo = (Channel << 4) | Device;
    if ( ControllerNo == SATACONTROLLER1 ) {
        switch ( PortNo ) {
            case 0x00: //PM
                TempStr[5] = '4';
                break;
