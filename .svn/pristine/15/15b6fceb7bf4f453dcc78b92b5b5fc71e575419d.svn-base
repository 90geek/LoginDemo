...
sCopyright (c) 1996 - 2016, Intel Corporation.
...
--*/
// AptioV server override start: Including Token.h here so that token.h gets included first and redefintion errors are avoided when including token.h
#include <Token.h>
// AptioV server override end: Including Token.h here so that token.h gets included first and redefintion errors are avoided when including token.h
#include <Base.h> 
#include <PiPei.h>
#include <Uefi.h>
  ...
  )
{
  ...
  //
  Data = 0;
  CpuIo->IoWrite8 (PeiServices, CpuIo, PM_BASE_ADDRESS + 0x30, Data); //AptioV server override: Use SDL token for PM base address
  ...
  IN VOID                      *Ppi)
{
...
#if UPSERVER_PLATFORM_SUPPORT
  //Aptio V server override - Start: Making changes according to Grantley ME-BIOS specification.
  // Grantley ME-BIOS spec, does not mention about CPU Dynamic Fusing procedure. so commenting it. 
  #if 0
  ...
  #endif
    //Aptio V server override - End: Making changes according to Grantley ME-BIOS specification.  
#endif // UPSERVER_PLATFORM_SUPPORT
