...
#include <Library/PlatformHooksLib.h> //AptioV server override: Added to use IsSoftSdvPlatform
...
//TH0053<<<
#include <Guid/SetupVariable.h> // AptioV Server Override : Changes done for EN1K Feature : EIP#206670
// AptioV Server Override Start : Changes done to resolve the legacy boot issue when enabling EN1K Feature
#include <Protocol\IioUds.h>
EFI_IIO_UDS_PROTOCOL        *mIohUds; 
// AptioV Server Override End : Changes done to resolve the legacy boot issue when enabling EN1K Feature
...
//AptioV server override start: Do not allow PCI bus driver to change smbus controller I/O base address
//ICH8 smbus controller 
//{icNotBar,		0x8086,		0x283E,		0xFFFF,		0x0000,	 	tBarMaxType,0x0014, 	0,			0			},
{icNotBar,		0x8086,		0x8d22,		0xFFFF,		0x0000,	 	tBarIo                ,0x0020, 	0,			0			},
//AptioV server override end: Do not allow PCI bus driver to change smbus controller I/O base address
//RTL8111E
...
// AptioV Server Override: Added the SoftSDV flag as it was asserting in SoftSDV.
...
//#endif
	...
	BOOLEAN				Padd64Pf;
// AptioV Server Override Start: Changes done for EN1K Feature : EIP#206670
    UINT64				IOGRAN = P2P_BRG_IO_GRA;
...
//-----------------------------------------------
    ...
    if (EFI_ERROR (Status)) {
//    	EN1KGRA = FALSE;
    	IOGRAN  = P2P_BRG_IO_GRA;
    	PCI_TRACE((TRACE_PCI,"::IOGRAN:4k %lx",IOGRAN));
    }
    
    else
    {
    	if (SystemConfiguration.EN1K)
    	{
//    		EN1KGRA = TRUE;
    		
    	    //
    	    // As per Intel RC,s4032514 - Need to be able to set En1K, but can't set it for IIO0 because it may cause legacy boot issues.
            //    		
    	    // AptioV Server Override Start : Changes done to resolve the legacy boot issue when enabling EN1K Feature :     		
    	    if (mIohUds == NULL )
    	    {
    		...
    		if (EFI_ERROR(Status))
    		{
    		      	TRACE((TRACE_ALWAYS, "CalculateBrgResources Error:Unable to find IIO UDS Protocol\n"));
    		      	ASSERT_EFI_ERROR (Status);
    		       	
    		}
    	    }
   	    PCI_TRACE((TRACE_PCI,"Skipping EN1K for IIO devices () Bridge->[B%X|D%X|F%X] ---> BEGIN \n",
    				Brg->Address.Addr.Bus, Brg->Address.Addr.Device, Brg->Address.Addr.Function));
    	    if ((Brg->Address.Addr.Bus >= mIohUds->IioUdsPtr->PlatformData.IIO_resource[0].BusBase) && (Brg->Address.Addr.Bus <= mIohUds->IioUdsPtr->PlatformData.IIO_resource[0].BusLimit))
    	    {
    		PCI_TRACE((TRACE_PCI,"::IOGRAN:4k %lx",IOGRAN));
    		IOGRAN = P2P_BRG_IO_GRA;    
    	    }
    	    ...
    	    {
    		PCI_TRACE((TRACE_PCI,"::IOGRAN:1k %lx",IOGRAN));
    		IOGRAN = 0x3FF;
    	    }
            ...
            // AptioV Server Override End : Changes done to resolve the legacy boot issue when enabling EN1K Feature :     		
// AptioV Server Override End : Changes done for EN1K Feature : EIP#206670
    
	PCI_TRACE((TRACE_PCI,"::IOGRAN: %lx",IOGRAN));
	...
	PCI_TRACE((TRACE_PCI,"++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n"));
    	...
    	if(Brg->Type==tPciRootBrg){
    		...
    		ext->Align[1].MinGran=IOGRAN /*P2P_BRG_IO_GRA*/;// AptioV Server Override : Changes done for EN1K Feature : EIP#206670
    		ext->Align[2].MinGran=IOGRAN/*P2P_BRG_IO_GRA*/; // AptioV Server Override : Changes done for EN1K Feature : EIP#206670
    	...
    	ext->Res[rtIo16].Gran=IOGRAN/*P2P_BRG_IO_GRA*/;// AptioV Server Override : Changes done for EN1K Feature : EIP#206670
    	ext->Res[rtIo32].Gran=IOGRAN/*P2P_BRG_IO_GRA*/;// AptioV Server Override : Changes done for EN1K Feature : EIP#206670
...
//----------------------------------	
	...
	for(pcidev.Addr.Device=0; pcidev.Addr.Device<=PCI_MAX_DEVICE; pcidev.Addr.Device++){
		...
		for(pcidev.Addr.Function=0;  pcidev.Addr.Function<=PCI_MAX_FUNC; pcidev.Addr.Function++){
            ...
            ///
//Aptiov Server override -Wrong LPC DeviceId in Simics- Start 			
#ifndef SOFTSDV_FLAG
			Status=PciCfg32(ParentBrg->RbIo,pcidev,TRUE,&id); 
			ASSERT_EFI_ERROR(Status);
			if(EFI_ERROR(Status)) return Status; //Error happend - can't continue
#endif			
//Aptiov Server override -Wrong LPC DeviceId in Simics- End 						
...
EFI_STATUS EnumerateAll(EFI_HANDLE Controller){
	...
	UINT64                     RecordId;
	UINT8 Record = EFI_EVENT_LOG_TYPE_PCI_BUS_OUT_OF_RESOURCES;   //Example for Error Type. You could modify for need.
	//TH0053<<<
...
//OUT_OF_RES!!  
        ...
        //TH0053  if(!EFI_ERROR(Status)) ERROR_CODE(DXE_PCI_BUS_OUT_OF_RESOURCES,EFI_ERROR_MAJOR);
        //TH0053>>>
        if(!EFI_ERROR(Status)){
        	Status = pBS->LocateProtocol (
...
//------------------------
	...
	if(notarom || !RomMatch){
		...
		Dev->PciIo.RomSize=0;
		Status=EFI_NOT_FOUND;
	} else Status=EFI_SUCCESS; //AptioV Server override: Fix for No display issue with Dual GTX590 VGA
