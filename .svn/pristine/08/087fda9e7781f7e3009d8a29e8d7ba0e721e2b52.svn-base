...
)
{
    ...
    if ( gHddSecurityCount != 0 && HddSecurityInternalData != NULL ) {
        ...
        //
        MemCopy( HddSecurityInternalData, TempIDEPasswordSecurityData, sizeof(HDD_SECURITY_INTERNAL_DATA) * gHddSecurityCount );
        MemFreePointer((VOID**)&TempIDEPasswordSecurityData );
        DataPtr = HddSecurityInternalData;
        //
        // DataPtr moved to free Entry
        //
        DataPtr+=gHddSecurityCount;
        HDDCount=gHddSecurityCount;
...
)
{
    ...
    if(HddSecurityInternalData != NULL) {
        //
        // Clear the Password
        //
        MemSet( HddSecurityInternalData, sizeof(HDD_SECURITY_INTERNAL_DATA) * gHddSecurityCount, 0);
        //
        // Free up the Memory
        //
        MemFreePointer((VOID**)&HddSecurityInternalData );
...
)
{
    ...
    for ( i = 0; i < gHddSecurityCount; i++ ) {
        UpdateSetupDataByHddIndex( ptrHddSecurityConfig, i);
    }
    return;
}
