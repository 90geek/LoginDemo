    ...
    UINT16      LangIndex)
{
    ...
    if (DescBuffer == NULL || ((USB_DESCRIPTOR_T*)Buffer)->DescriptorType != Type) {
        return NULL;
    }
    gBS->AllocatePool(EfiBootServicesData, ((USB_DESCRIPTOR_T*)Buffer)->Length, &Desc);

	if (((USB_DESCRIPTOR_T*)Buffer)->Length <= sizeof(Buffer)) {
...
FindIadDetails(DEVGROUP_T* Grp)
{
    ...
    USB_DEBUG(3, "IAD details: totally %d IADs:\n", Grp->iad_count);
    IadDetails = Grp->iad_details;
    for (i = 0; i < Grp->iad_count; i++, IadDetails++)
    {
        USB_DEBUG(3, " IAD#%d. ConfigIndex %d, IAD Descriptor Ofs %x-%x=%x (%d), ifFrom %d ifCount %d\n",
...
)
{
    ...
    EFI_STATUS Status;
    USB_DEBUG(3, "EnableEndpoints: HcType is %x\n", Device->hc_info->bHCType);
...
)
{
    ...
    if ((Dev->LoadedAltIfMap & (1 << AltIfSetting)) == 0)
    {
        ...
        for (Offset = 0; NextDescriptor(CfgDesc, &Offset);)
        {
            ...
            {
                USB_DEBUG(DEBUG_LEVEL_USBBUS, "\t...IF found.\n" );
    ...
    if (AltIfSetting != 0)
    {
        USB_DEBUG(DEBUG_LEVEL_USBBUS,"\t\t...Enable endpoints for ALT_IF_SETTING %d...\n", AltIfSetting);
...
)
{
    ...
    if (!(Dev->dev_info->Flag & DEV_INFO_DEV_PRESENT)) {
        USB_DEBUG(3, "UsbIoIsochronousTransfer ERROR: device is not present\n");
        return EFI_DEVICE_ERROR;
    }
    ...
    if ( Endpoint != Dev->dev_info->IsocDetails.Endpoint ) {
        USB_DEBUG(3, "UsbIoIsochronousTransfer ERROR: Endpoint %x whereas dev_info->IsocEndpoint %x\n",
    ...
    return Dev->hc->IsochronousTransfer(
        Dev->hc, Dev->dev_info->bDeviceAddress, Endpoint, GetSpeed(Dev),
        Dev->dev_info->IsocDetails.EpMaxPkt, 1, &Data, DataLength, NULL, Status);
}
...
)
{
    ...
    if (!(Dev->dev_info->Flag & DEV_INFO_DEV_PRESENT)) {
        USB_DEBUG(3, "UsbIoAsyncIsochronousTransfer ERROR: device is not present\n");        
        return EFI_DEVICE_ERROR;
    }
    ...
    if ( Endpoint != Dev->dev_info->IsocDetails.Endpoint ) {
        USB_DEBUG(3, "UsbIoAsyncIsochronousTransfer ERROR: Endpoint %x whereas dev_info->IsocEndpoint %x\n",
                Endpoint, Dev->dev_info->IsocDetails.Endpoint);
        return EFI_UNSUPPORTED;
    }
...
)
{
    ...
    if (Dev->descIF[Dev->dev_info->bAltSettingNum] == NULL) {
        USB_DEBUG(3, "IF NOT FOUND on UsbIo %x Dev %x\n", UsbIo, Dev);
        return EFI_NOT_FOUND;
    }
...
{    
    ...
    USB_DEBUG(3,"ProcessIad: iad_count %d\n", Grp->iad_count);
    ...
    for (i = 0; i < Grp->iad_count; i++, IadDetails++)
    {
        ...
        USB_DEBUG(3,"ProcessIad[%d]: intrf %d (%d...%d)\n", i, Interface, IfFrom, IfTo);
        ...
        if (Interface >= IfFrom && Interface <= IfTo)
        {
            ...
            if (j == Grp->iad_details[i].Descriptor->bInterfaceCount)
            {
                ...
                Status = gBS->InstallMultipleProtocolInterfaces(
                        ...
                        NULL);
                ASSERT_EFI_ERROR(Status);
                USB_DEBUG(3, "ProcessIad: Install IadIo protocol: %r\n", Status);
...
{
    if (node == 0) return;
    USB_DEBUG(3,"%x <- %x -> %x, child %x parent %x\n",
        ...
        if ((Grp->dev_desc.StrManufacturer != 0) || (Grp->dev_desc.StrProduct != 0) || 
            ...
            if (Grp->lang_table && Grp->lang_table->len != 0) {
                ...
                if (Index != Grp->lang_table->len) {
                    ...
                    // whcih describing manufacturer, product and the device's 
                    // serial number, set the timeout value to 100 ms.
...
)
{
    ...
    Dev->dev_info->bIntEndpoint = IntEndpoint;
    Dev->dev_info->wIntMaxPkt = IntMaxPktSize;
...
EFI_STATUS UsbAccumulateIsochronousData(
    EFI_USB_PROTOCOL*   AmiUsbProtocol,
    VOID*   UsbIo,
    UINT8*  Buffer,
    UINT8   IsocEndpoint,
