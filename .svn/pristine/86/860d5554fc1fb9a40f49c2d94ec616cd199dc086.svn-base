...
UINT16  BulkTransferTimeOut = 0;  // Timeout variable to get timeout provided in PCD
...
BotClearEndpoint (
    ...
    if( EFI_ERROR(Status) ) {
       SERVER_IPMI_DEBUG((EFI_D_LOAD, "BotClearEndpoint Error : UsbStatus = %x\n",UsbStatus));
    }
    return Status;
  ...
  IN  EFI_IPMI_USB_INSTANCE_DATA    *IpmiUsbInstance )
{
    ...
    if ( EFI_ERROR(Status) ) {
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "BotRecoveryReset Error : UsbStatus = %x\n",UsbStatus));
        return Status;
    }
  ...
  IN  EFI_IPMI_USB_INSTANCE_DATA    *IpmiUsbInstance )
{
    ...
    if( EFI_ERROR(Status) ) {
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "Dxe BotStatusPhase Error : Status = %r UsbStatus = %x\n",Status,UsbStatus));
        ...
        if( EFI_ERROR(Status) ) {
            SERVER_IPMI_DEBUG((EFI_D_LOAD, "BotStatusPhase Error : Performing RecoveryReset\n"));
  ...
  IN  UINT8                         *RequestData )
{
    ...
    while ( RemainingDataBytes != 0 ) {
        ...
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "IpmiRequestSize: %x \n", IpmiRequestSize));
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "IpmiRequestOffset: %x \n", IpmiRequestOffset));
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "RequestDataOffset: %x \n", RequestDataOffset));
        ...
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "RemainingDataBytes: %x \n", RemainingDataBytes));
  ...
  IN  UINT32                        DataSize )
{
    ...
    SERVER_IPMI_DEBUG((EFI_D_LOAD, "DataSize: %x \n", DataSize));
    ...
    Status = BotCommandPhase (
                ...
                Sector,
                DataSize );
    SERVER_IPMI_DEBUG((EFI_D_LOAD, "BotCommandPhase: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
    ...
    if ( (Opcode == COMMON_READ_10_OPCODE ) && \
        ...
        Status = ReadResponseData (
                    ...
                    DataSize,
                    Data );
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "ReadResponseData: Status: %r \n", Status));
        if ( EFI_ERROR(Status) ) {
            return Status;
        }
        ...
        Status = WriteCommandData (
                    ...
                    DataSize,
                    Data );
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "WriteCommandData: Status: %r \n", Status));
        if ( EFI_ERROR(Status) ) {
            return Status;
        }
    } else {
        ...
        Status = UsbIo->UsbBulkTransfer (
                    ...
                    BulkTransferTimeOut,
                    &UsbStatus );
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "UsbIo->UsbBulkTransfer: Status: %r \n", Status));
        if ( EFI_ERROR(Status) ) {
            return Status;
        }
    }
    ...
    SERVER_IPMI_DEBUG((EFI_D_LOAD, "BotStatusPhase: Status: %r \n", Status));
    if ( EFI_ERROR(Status)) {
        return Status;
    }
  ...
  IN  UINT32                        DataSize )
{
    ...
    Status = IssueBmcUsbTransaction (
                ...
                (UINT8*)&CmdHeader,
                DataTransferSize );
    SERVER_IPMI_DEBUG((EFI_D_LOAD, "Send the Command header: IssueBmcUsbTransaction: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
    ...
    Status = IssueBmcUsbTransaction (
                ...
                IpmiRequest,
                DataTransferSize );
    SERVER_IPMI_DEBUG((EFI_D_LOAD, "Send the IPMI packets : IssueBmcUsbTransaction: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
    ...
    do {
        ...
        Status = IssueBmcUsbTransaction (
                    ...
                    (UINT8*)&CmdHeader,
                    DataTransferSize );
        SERVER_IPMI_DEBUG((EFI_D_LOAD, "Wait for command completion : IssueBmcUsbTransaction: Status: %r \n", Status));
        if ( EFI_ERROR(Status) ) {
            return Status;
        }
        //
    ...
    switch ( CmdHeader.Status ) {
        case ERR_SUCCESS:
            ...
            SERVER_IPMI_DEBUG((EFI_D_LOAD,"gResponseDataSize: %lx \n", gResponseDataSize));
            break;
        case ERR_BIG_DATA:
            SERVER_IPMI_DEBUG((EFI_D_LOAD,"Too much data send to IPMI over USB\n"));
            Status = EFI_INVALID_PARAMETER;
            break;
        case ERR_NO_DATA:
            SERVER_IPMI_DEBUG((EFI_D_LOAD,"Too little data send to IPMI over USB\n"));
            Status = EFI_INVALID_PARAMETER;
            break;
        case ERR_UNSUPPORTED:
            SERVER_IPMI_DEBUG((EFI_D_LOAD,"Unsupported command\n"));
            Status = EFI_UNSUPPORTED;
            break;
        default:
            SERVER_IPMI_DEBUG((EFI_D_LOAD,"Unknown status\n"));
  ...
  IN  UINT32                         *IpmiResponseDataSize )
{
    ...
    SERVER_IPMI_DEBUG((EFI_D_LOAD, "ReceiveDataFromUsbBmc: DataTransferSize: %x \n", DataTransferSize));
    ...
    Status = IssueBmcUsbTransaction (
                ...
                ResponseData,
                DataTransferSize );
    SERVER_IPMI_DEBUG((EFI_D_LOAD, "Get the IPMI response: IssueBmcUsbTransaction: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
    ...
    Status = IssueBmcUsbTransaction (
                ...
                (UINT8*)&CmdHeader,
                DataTransferSize );
    SERVER_IPMI_DEBUG((EFI_D_LOAD, "Send configuration command: IssueBmcUsbTransaction: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
  ...
  OUT UINT32                    *ResponseDataSize )
{
    ...
    //
    BulkTransferTimeOut = PcdGet16 (BulkDataTransferTimeout);
