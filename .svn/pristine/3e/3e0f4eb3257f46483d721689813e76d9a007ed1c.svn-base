...
--*/
{
  ...
  // Intel has found a potential workaround for the MCE Bank 5 and Bank 20 Correctable Error issue for A step Broadwell-EP.. \
  // Please work with your BIOS vendor to create a patch with the following lines in green commented out. We have tested this on
  // our CRB running both linpack and stress test and found no MCE Bank 5 and 20 errors running at 9.6GT/s. sa Other POR speeds are being tested.
...
--*/
{
  ...
  if ((host->var.common.cpuType == CPU_BDX) && (QpiInternalGlobal->SkuType[sbspSktId] == SKU_DE)) {
...
--*/
{
  ...
  UINT8   TempTotCpu;
  UINT32  TempCpuList;
  //AptioV server override start: Suppress QPI debug message in release mode build
#ifdef SERIAL_DBG_MSG  
  UINT8   Topology[SYS_CONFIG_MAX][5] = {{"1S"}, {"2SEN"}, {"2SEP"}, {"4S"}, {"8S"}};
#endif
  //AptioV server override end: Suppress QPI debug message in release mode build  
...
--*/
{
  ...
  if ((host->var.qpi.SysConfig != SYS_CONFIG_1S && host->var.qpi.SysConfig != SYS_CONFIG_2SEP)) {
    host->var.qpi.OutIsocEn = FALSE;
  } else if ((host->var.common.cpuType == CPU_BDX) && (QpiInternalGlobal->SkuType[QpiInternalGlobal->SbspSoc] == SKU_DE)) {
    host->var.qpi.OutIsocEn = FALSE;
  }
   ...
   for (Soc = 0; Soc < MAX_CPU_SOCKETS; ++Soc) {
     if (SocketData->Cpu[Soc].Valid == TRUE) {
        ...
        if ((host->var.common.cpuType == CPU_BDX) && (host->var.common.stepping >= B0_REV_BDX) && 
            (QpiInternalGlobal->SkuType[QpiInternalGlobal->SbspSoc] == SKU_DE)) {
