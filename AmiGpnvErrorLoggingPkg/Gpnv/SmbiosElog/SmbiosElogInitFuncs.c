...
#include <AmiCompatibilityPkg\Include\Dxe.h>
...
//TH0095 >>>>
EFI_STATUS
InsertClearCmosRecord (VOID);
EFI_STATUS Is_CMOS_Clear(VOID);
//TH0095 <<<<
...
//TH0018    UINTN                             TempAddress = LOCAL_EVENT_LOG_PHY_ADDR;
    UINTN	 TempAddress = LOCAL_EVENT_LOG_PHY_ADDR + sizeof(EFI_FIRMWARE_VOLUME_HEADER); //TH0018
...
// Register CallbackEvent for gAmiSmbiosProtocolGuid.
    ...
    //TH0095 >>>>
    //
    // Insert Clear CMOS Record
    //
    Status = Is_CMOS_Clear();
    if(Status == EFI_SUCCESS){
        InsertClearCmosRecord ();
     }
    //TH0095 <<<<
...
EFI_STATUS Is_CMOS_Clear(VOID)
{
    ...
    NVRAM_HOB *NvramHob = (NVRAM_HOB*)GetEfiConfigurationTable(gST,&guidHobList);
    if (!NvramHob) return EFI_ABORTED;
    if (EFI_ERROR(FindNextHobByGuid((EFI_GUID*)&AmiNvramHobGuid, &NvramHob))) return EFI_ABORTED;
...
EFI_STATUS InsertClearCmosRecord (VOID)
{
	EFI_STATUS                	Status = EFI_SUCCESS;  
	UINT64                    	RecordId;
	UINT8 Record = EFI_EVENT_LOG_TYPE_CLEAR_CMOS;
