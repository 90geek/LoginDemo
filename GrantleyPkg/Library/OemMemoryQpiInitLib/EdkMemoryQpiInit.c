...
)
{
	...
	UINT16								DimmPresent = 0;
	UINT8								SetDimmInfo[4] = {0x01, 0x82, 0x00, 0x00};
	...
	if (!EFI_ERROR(Status)) {	
		SetDimmInfo[2] = (UINT8)DimmPresent;
		SetDimmInfo[3] = (UINT8)(DimmPresent >> 8);
		DataSize = sizeof(ResponseData);
...
// AptioV server override end
  ...
  for(i = 0; i < SAD_RULES; i++) {
    ...
    if((mem->SAD[i].Enable) && (mem->SAD[i].IotEnabled != 1)) { //To ensure that IOT is not reported to systemMemoryMap/SRAT, other tables
      ...
      if(i) {
        systemMemoryMap.Element[ScratchCounter].BaseAddress = mem->SAD[i-1].Limit;
...
//#ifdef SDP_FLAG AptioV server override: SMBIOS type 17 data is independent of flag in AMI BIOS
...
//#ifdef SDP_FLAG	AptioV server override: Remove flag dependency to report all data in type 17	
        for (i = 0; i < 4; i++) {
...
//#endif //SDP_FLAG AptioV server override: Remove flag dependency to report all data in type 17
        for (rank = 0; rank < MAX_RANK_DIMM; rank++) {
          ...
          systemMemoryMap.NodeInfo[node].ChannelInfo[haCh].DimmInfo[dimm].mapOut[rank] = host->nvram.mem.socket[socket].channelList[ch].dimmList[dimm].mapOut[rank];
...
//TH0114>>>		   
#if REPORT_MEM_CONFIG_TO_BMC  
  SendMemInfoToBMC(systemMemoryMap, host);	
#endif
//TH0114<<<
...
#ifdef LT_FLAG
  if (PlatformInfo->MemData.MemLtMemSize > 0) {	//Aptio V Server Override start- Add if state ment
