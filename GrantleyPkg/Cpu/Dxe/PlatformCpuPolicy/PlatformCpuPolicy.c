...
--*/
{
  ...
  if(Status != EFI_SUCCESS) {
    ...
    i = (UINT64)(*(UINT64 *)(&Addr)); // AptioV Server Override: To resolve system hang at exception D if EFI_PPM_STRUCT was allocated a base which is above 4G.
...
//Aptio V Override. The errata is for Haswell A-0 is that 0x1fc default is incorrect.
//However, this workaround is wrong because it overrides setup values.
#if 0
  //WA 4165788
  //AptioV Server Override: Cpu Signature check correction.
  if((((CpuFamilyModelStepping & 0xFFFFFFF0) >> 4) == CPU_FAMILY_HSX) || (((CpuFamilyModelStepping & 0xFFFFFFF0) >> 4) == CPU_FAMILY_BDX) || (((CpuFamilyModelStepping & 0xFFFFFFF0) >> 4) == CPU_FAMILY_BDX_DE)) {
